...
create table jsoncol(
  majorKey1 string,
  majorKey2 string,
  minorKey string,
  primary key(shard(majorKey1,majorKey2),minorKey)) as json collection using ttl 5 days

create index idx_index on jsoncol(index as long)

create index idx_name on jsoncol(address.name as string)

create table foo (id integer, primary key(id)) as json collection
