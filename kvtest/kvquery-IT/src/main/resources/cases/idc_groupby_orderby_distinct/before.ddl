...
create table if not exists SimpleDatatype(id integer,shardId integer, name string, salary double, percentage float, rank long, rating double, isEmp boolean, enm ENUM(tok1,tok2,tok3), lastname string, age integer, bin BINARY, fbin BINARY(10), primary key (shard(shardId),id))
