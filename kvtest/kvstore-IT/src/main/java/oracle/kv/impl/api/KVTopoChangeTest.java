...
/** Test KV operations during topology changes. */
public class KVTopoChangeTest extends TestBase {
            ...
            try {
                ...
                for (int i = 0; !stop; i++) {
                    final KeyValue kv = new KeyValue(
                        Key.createKey("key" + i),
                        Value.createValue(new byte[] { (byte) i }));
                    final SingleKVStream stream = new SingleKVStream(kv);
                    store.put(singletonList(stream),
    ...
    /** Stream with one KeyValue */
    static class SingleKVStream implements EntryStream<KeyValue> {
        final KeyValue keyValue;
        private boolean supplied;
        private boolean completed;

        SingleKVStream(KeyValue keyValue) {
        ...
        @Override
        public String name() {
            return "SingleRowStream[" + keyValue + "]";
        }

        @Override
        public synchronized KeyValue getNext() {
            if (supplied) {
                return null;
            }
            supplied = true;
        ...
        @Override
        public void keyExists(KeyValue kv) {
            fail("Unexpected call to keyExists: " + kv);
        }

        @Override
        public void catchException(RuntimeException e, KeyValue kv) {
            throw new RuntimeException(
                "Unexpected call to catchException for entry " + kv + ": " + e,
                e);
        }
    }
}
