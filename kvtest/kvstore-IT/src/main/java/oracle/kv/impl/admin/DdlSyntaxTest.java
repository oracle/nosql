...
public class DdlSyntaxTest extends TestBase {
    ...
    public void testFunctionalIndex() {
        ...
        /* idx0: functional index */
        String[] idx0Fields = new String[] {
            "s",
            "ts",
            "expiration_time#"
        };
        String[] idx0ExternalFields = new String[] {
            "s",
            "ts",
            "expiration_time()"
        };
        checkFunctionalIndex(table, "idx0", idx0Fields, null,
                             idx0ExternalFields, false, true);

        /* idx1: functional index */
        String[] idx1Fields = new String[] {
            "length#s",
            "substring#s@,1,2",
            "power#i@,2",
        ...
        String[] idx1ExternalFields = new String[] {
            "length(s)",
            "substring(s,1,2)",
            "power(i,2)",
            "timestamp_round(ts,2)",
        ...
        checkFunctionalIndex(table, "idx1", idx1Fields, null,
                             idx1ExternalFields, true, false);

        /* idx2: functional index on json fields */
        String[] idx2Fields = new String[] {
            "trunc#j.n.d@,3",
            "substring#j.m[].values().name@,3",
            "power#j.\"#\".a.\"@\"@,2"
        };
        FieldDef.Type[] idx2Types = new FieldDef.Type[] {
            FieldDef.Type.DOUBLE,
            FieldDef.Type.ANY_ATOMIC,
            FieldDef.Type.INTEGER
        };
        String[] idx2ExternalFields = new String[] {
            "trunc(j.n.d AS Double,3)",
            "substring(j.m[].values().name AS AnyAtomic,3)",
            "power(j.\"#\".a.\"@\" AS Integer,2)"
        };
        checkFunctionalIndex(table, "idx2", idx2Fields, idx2Types,
                             idx2ExternalFields, true, false);

        /* idx3: index on json field */
        String[] idx3Fields = new String[] {
            "j.n.d",
            "j.m[].values().name",
            "j.m[].keys()"
        };
        FieldDef.Type[] idx3Types = new FieldDef.Type[] {
            FieldDef.Type.INTEGER,
            FieldDef.Type.STRING,
            null
        };
        String[] idx3ExternalFields = new String[] {
            "j.n.d AS Integer",
            "j.m[].values().name AS String",
            "j.m[].keys()"
        };
        checkFunctionalIndex(table, "idx3", idx3Fields, idx3Types,
                             idx3ExternalFields, true, false);
    }

    private void checkFunctionalIndex(TableImpl table,
                                      String indexName,
                                      String[] internalFields,
                                      FieldDef.Type[] types,
                                      String[] externalFields,
                                      boolean indexNulls,
                                      boolean isUnique) {

        IndexImpl impl = new IndexImpl(indexName, table,
                                       Arrays.asList(internalFields),
                                       (types != null ?
                                            Arrays.asList(types) : null),
                                       indexNulls, isUnique, null);
        table.addIndex(impl);

        /* Verify the index ddl generated by DDLGenerator */
        String ddl = genIndexDdl(indexName, table.getFullName(), externalFields,
                                 indexNulls, isUnique);

        DDLGenerator ddlGen = new DDLGenerator(table, false);
        List<String> indexDdls = ddlGen.getAllIndexDDL();
        assertTrue(indexDdls.size() > 0);
        assertEquals(ddl, indexDdls.get(indexDdls.size() - 1));

        /* Check the tabular format output of index */
        String result = impl.formatIndex(false);
        for (String field : externalFields) {
            assertTrue("Result=" + result + ", Field=" + field,
                       result.contains(field));
        }

        /* Check the JSON metadata of index */
        String tableJson = table.toJsonString(true);
        TableImpl newTable = TableJsonUtils.fromJsonString(tableJson, null);
        assertEquals(table.getIndexes().size(), newTable.getIndexes().size());
        assertEquals(table.getIndex(indexName), newTable.getIndex(indexName));
    }

    /* Returns create index ddl based on the provided information */
    private String genIndexDdl(String indexName,
                               String tableName,
                               String[] externalFields,
                               boolean indexNulls,
                               boolean isUnique) {
