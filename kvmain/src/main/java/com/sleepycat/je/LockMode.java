 ...
 * end of the transaction.  However, if {@link
 * TransactionConfig#setReadCommitted read-committed} is configured,
 ...
 * read-committed is used for a database (non-cursor) operation, the read
 * lock is released before the method returns.</p>
 *
 * <p>When neither {@link #READ_UNCOMMITTED} nor read-committed is
 * specified, read and write locking as described above provide Repeatable Read
 * isolation, which is the default transactional isolation level.</p>
 *
 * <p><strong>Non-Transactional Locking</strong></p>
 *
 * <p>Non-transactional operations include all operations for a
 * non-transactional database, and read operations for a transactional database
 * when a null {@link Transaction} parameter is passed.</p>
 *
 ...
 * is moved to a different record, when the cursor's state becomes
 * {@code uninitialized} due to failure of a cursor movement operation, or
 * when the cursor is closed.  For database (non-cursor) operations, the read
 * or write lock is released before the method returns.</p>
 *
 * <p>This behavior is similar to read-committed, except that both
 * read and write locks are released.</p>
 *
 ...
 * {@link TransactionConfig#setReadCommitted read-committed}.</p>
 *
...
public enum LockMode {
     ...
     *
     * <p>Otherwise, the default lock mode is read-committed when this
     * lock mode is configured via {@link TransactionConfig#setReadCommitted}.
     * The read-committed mode overrides other configuration settings except
     * for {@link #READ_UNCOMMITTED}.</p>
     *
    ...
    LockMode() {
        readOptions = new ImmutableReadOptions(this);
    }

    /**
     * Returns an immutable ReadOptions with this LockMode property, and
     * default values for all other properties.
     *
     * <p>Use clone() on the returned instance to get a modifiable copy.</p>
     *
     * @since 7.0
     */
    ...
    /**
     * Wrapper class to prevent accidental modification to the singleton.
     */
    private static class ImmutableReadOptions extends ReadOptions {

        ImmutableReadOptions(LockMode mode) {
            super.setLockMode(mode);
        }

        /**
         * Returns a mutable copy. Cannot call super.clone since the copy will
         * be of the same class and will be immutable.
         */
        @Override
        public ReadOptions clone() {
            return new ReadOptions().setLockMode(getLockMode());
        }
        ...
        private IllegalStateException getIllegalStateException() {
            return new IllegalStateException(
                "Cannot modify a constant singleton, use" +
                " clone() to get a modifiable copy.");
        }
    }
}
