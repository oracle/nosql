...
public abstract class DatabaseException extends RuntimeException {
    ...
    private final static String VERSION_HEADER =
        "(JE " + JEVersion.CURRENT_VERSION + ") ";
    ...
    public DatabaseException(String message) {
        super(addVersionHeader(message));
    }
    ...
    public DatabaseException(String message, Throwable t) {
        super(addVersionHeader(message), t);
    }
    ...
    /**
     * Avoid allocating another large string (e.g., an embedded stack trace)
     * when the version header is already present, which is especially
     * important for an OutOfMemoryError.
     */
    private static String addVersionHeader(String msg) {
        return msg.startsWith(VERSION_HEADER) ?
            msg : VERSION_HEADER + msg;
    }
    ...
    public void addErrorMessage(String newExtraInfo) {
        ...
        /* Avoid memory allocation for Errors. */
        if (isJavaError()) {
            return;
        }
     ...
     * @hidden
     *
     * <p>Adds the current stack trace to the exception message, before it is
     ...
     * approach rather than wrapSelf when user code relies on the getCause
     * method to return a specific exception, and cloning would change the
     * cause exception to something unexpected.</p>
     *
    ...
    public void addRethrownStackTrace() {
        ...
        /* rethrownStackTraces is transient and may be null. */
        if (rethrownStackTraces == null) {
            return;
        }
    ...
    public String getMessage() {
        ...
        /* rethrownStackTraces is transient and may be null. */
        final String rethrownStackTrace =
            (rethrownStackTraces != null) ? rethrownStackTraces.get() : null;
