...
public class Window {
    ...
    public Window(Cursor cursor, int windowSize)
        ...
        /* To compute a SHA-256 hash for each block. */
        try {
            md = MessageDigest.getInstance("SHA-256");
        } catch (NoSuchAlgorithmException e) {
            System.err.println(e.getMessage());
            throw new Exception("SHA-256 hashes are required for ldiff.");
        }
     ...
     * account for this move past one key/value pair. Note that the SHA-256
     * hash associated with the window is computed on demand and is not updated
     * here.
     */
    ...
    private boolean getNext(DatabaseEntry key,
                            DatabaseEntry data) {
        if (noMoreRecords) {
            return false;
        }
    ...
    /**
     * Compute the SHA-256 hash for the window. This is an expensive operation
     * and should be used sparingly.
     * 
     * @return the SHA-256 for the window.
     */
