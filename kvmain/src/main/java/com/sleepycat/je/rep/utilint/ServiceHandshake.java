...
public class ServiceHandshake {
    ...
    public static enum InitResult {
        ...
        READ,
        /* Need to wait for channel write available */
        WRITE_AND_FLUSH,
        /* Need to execute tasks for SSL */
        EXECUTE_TASKS,
    ...
    public abstract static class ServerInitOp {
         ...
         *    InitResult.WRITE_AND_FLUSH: operation is not complete; need to
         *      write data, but the channel is busy
         *    InitResult.EXECUTE_TASKS: operation is not complete; some
         *      handshake tasks need to be executed
        ...
        protected InitResult fillBuffer(DataChannel channel, ByteBuffer buffer)
            ...
            while (buffer.remaining() > 0) {
                ...
                if (count == 0) {
                    final AsyncIO.ContinueAction action =
                        channel.getAsyncIOContinueAction(AsyncIO.Type.READ);
                    switch(action) {
                    case RETRY_NOW:
                    case WAIT_FOR_CHNL_READ:
                        return InitResult.READ;
                    case WAIT_FOR_CHNL_WRITE_THEN_FLUSH:
                        return InitResult.WRITE_AND_FLUSH;
                    case WAIT_FOR_TASKS_EXECUTION:
                        return InitResult.EXECUTE_TASKS;
                    case APP_READ:
                        throw new IllegalStateException(
                                      String.format(
                                          "Wrong action %s when read",
                                          action));
                    default:
                        throw new IllegalStateException(
         ...
         */
        void closeChannel(DataChannel channel) {
            if (channel != null) {
                try {
                    /*
                     * Close data channel forcefully for non-blocking channel.
                     * A non-blocking clean closeAsync is more preferrable, but
                     * we do not have the mechanism here to do a clean close.
                     * It seems problematic to block here and do a clean close
                     * because we are in the selector loop, so we do a forceful
                     * close. Also data channel writes attempt to flush
                     * themselves, so no need to flush again.
                     */
