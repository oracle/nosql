...
public class RepGroupImpl {
     ...
     */
    public final static UUID UNKNOWN_UUID = new UUID(0, 0);
    ...
    private final Map<Integer, RepNodeImpl> nodesById = new HashMap<>();
     ...
     */
    private final Map<String, RepNodeImpl> nodesByName = new HashMap<>();
    ...
    /*
     * Cache used to hold slow-changing summary values calculated based upon
     * the structure of the group. SummaryCache.refresh() must be invoked to
     * recompute the summary after any updates to the group.
     */
    private final SummaryCache cache = new SummaryCache();
    ...
    public void setUUIDNoCheck(UUID uuid) {
        this.uuid = uuid;
    }
    ...
    /**
     * Includes the electable nodes from another RepGroupImpl.
     *
     * This method is called in a special case where this group is created from
     * an empty log file and is not properly initialized yet. Includes the
     * electable nodes from another group representation obtained from other
     * nodes so that we could continue with the same electable nodes for
     * election.
     */
    public void includeElectableMembers(RepGroupImpl otherGroup) {
        if (!hasUnknownUUID()) {
            throw new IllegalStateException(String.format(
                "Cannot include other group members " +
                "when this group %s is initialized with a UUID",
                groupName));
        }
        setNodes(
            otherGroup.getAllElectableMembers().stream()
            .map((n) ->
                 new AbstractMap.SimpleImmutableEntry
                 <Integer, RepNodeImpl>(n.getNodeId(), n))
    ...
    private boolean isValidFormatVersion(int version) {
        if (version > MAX_FORMAT_VERSION || version < MIN_FORMAT_VERSION
            || version > getMaxFormatVersion(minJEVersion)) {
            return false;
        }
        return true;
    }

    public void setFormatVersionNoCheck(int version) {
        formatVersion = version;
    }

    public void setFormatVersion(int version) {
        if (!isValidFormatVersion(version)) {
            throw new IllegalArgumentException("Invalid format version.");
        }
        setFormatVersionNoCheck(version);
    }
    ...
    public void setChangeVersion(int version) {
        for (RepNodeImpl mi : nodesById.values()) {
            /* Get the highest change version. */
            if (version < mi.getChangeVersion()) {
                throw new IllegalArgumentException(
                    "Submitted version: " + version
                    + " is less than existing node change version: "
                    + mi.getChangeVersion());
            }
        }
        setChangeVersionNoCheck(version);
    }

    public void setChangeVersionNoCheck(int version) {
    ...
    public void setNodeIdSequence(int nodeIdSequence) {
        if (nodeIdSequence < 0 || nodeIdSequence > NODE_SEQUENCE_MAX) {
            throw new IllegalArgumentException(
                "The id is less than the current maximum id.: " +
                nodeIdSequence);
        }
        setNodeIdSequenceNoCheck(nodeIdSequence);
    }

    public void setNodeIdSequenceNoCheck(int nodeIdSequence) {
    ...
    public void setMinJEVersion(final JEVersion newMinJEVersion)
        ...
        setMinJEVersionNoCheck(newMinJEVersion);
    }

    public void setMinJEVersionNoCheck(final JEVersion version) {
        minJEVersion = version;
        formatVersion = getMaxFormatVersion(version);
    }
    ...
    static public RepGroupImpl deserializeHex(String[] tokens, int start) {
        ...
        Map<Integer, RepNodeImpl> nodeMap = new HashMap<>();
     ...
     * Returns IDs for all ELECTABLE nodes that are not removed, even if they
     * are not acknowledged, but not for SECONDARY or EXTERNAL nodes.
     */
     ...
     * ELECTABLE nodes that are acknowledged and not removed, and SECONDARY and
     * EXTERNAL nodes. If the predicate is not null, only includes members that
     ...
     * set. Adds all ELECTABLE nodes that are not removed, even if they are not
     ...
     * Counts all ELECTABLE nodes that are not removed, even if they are not
     * acknowledged, and SECONDARY and EXTERNAL nodes. If the predicate is not
     ...
     * Adds ELECTABLE nodes that are not removed and are acknowledged, and
     * SECONDARY and EXTERNAL nodes. If the predicate is not null, only adds
     ...
     * socket address. Returns ELECTABLE nodes that are not removed, even if it
     * is not acknowledged, and SECONDARY and EXTERNAL nodes.
     *
    ...
    /**
     * Returns nodes that are removed from the group. Returns ELECTABLE nodes
     * that are removed and are acknowledged, but not SECONDARY or EXTERNAL
     * nodes, which are not remembered when they are removed.
     */
    public Set<RepNodeImpl> getRemovedNodes() {
        Set<RepNodeImpl> ret = new HashSet<>();
        synchronized (nodesById) {
            for (RepNodeImpl mi : nodesById.values()) {
     ...
     * acknowledged, and SECONDARY nodes, but not EXTERNAL nodes.
     */
     ...
     * removed and are acknowledged, and SECONDARY nodes, but not EXTERNAL
     * nodes.
     */
     ...
     * ELECTABLE nodes that are not removed, even if they are not
     * acknowledged, but not SECONDARY or EXTERNAL nodes.
     */
     ...
     *
     * Returns SECONDARY nodes, but not ELECTABLE or EXTERNAL nodes.
     */
     ...
     *
     * Returns EXTERNAL nodes, but not ELECTABLE or SECONDARY nodes.
     */
     ...
     * specified set. Adds SECONDARY nodes, but not ELECTABLE, or EXTERNAL
     * nodes.
     */
    ...
    /**
     * Adds the external nodes. Adds EXTERNAL nodes, but not ELECTABLE, or
     * SECONDARY nodes.
     */
     ...
     * the group. Returns addresses for all ELECTABLE nodes that are not
     * removed, even if they are not acknowledged, and for all nodes with
    ...
    private Set<InetSocketAddress> getAllMemberSockets(Predicate p) {
        Set<InetSocketAddress> sockets = new HashSet<>();
        synchronized (nodesById) {
            for (final RepNodeImpl mi : nodesById.values()) {
                if (((mi.getType().isElectable() && !mi.isRemoved()) ||
     ...
     * Returns addresses for all ELECTABLE nodes that are not removed, even if
     * they are not acknowledged, but not for SECONDARY or EXTERNAL nodes.
     *
     ...
     * addresses for all ELECTABLE nodes that are not removed, even if they are
     * not acknowledged, and SECONDARY nodes.
     *
     ...
     * but not for SECONDARY, or EXTERNAL nodes, which do not act as acceptors.
     *
     ...
     * member. Returns ELECTABLE nodes that are not removed, even if they are
     * not acknowledged, and SECONDARY and EXTERNAL nodes.
     *
     ...
     * state. Returns all ELECTABLE nodes, even if they are removed or are not
     * acknowledged, and SECONDARY and EXTERNAL nodes.
     *
     ...
     * they are not acknowledged, but not SECONDARY, or EXTERNAL nodes. Note
    ...
    public int getElectableGroupSize() {
        return cache.getElectableGroupSize();
    }

    /**
     * Returns the number of all electable nodes that are currently members of
     * the group. Includes all ELECTABLE nodes that are not removed, even if
     * they are not acknowledged, but not ARBITER, SECONDARY, or EXTERNAL
     * nodes. Note that even unACKed nodes are considered part of the group for
     * group size/durability considerations.
     *
     * @return the size of the group for durability considerations
     */
    public int getAckGroupSize() {
        return cache.getAckGroupSize();
    }
    ...
    /* Simple cache used to maintain summary values, to avoid recalculations
     * of these values on heavily used transaction paths.
     */
    private class SummaryCache {

        /* Cached value for electable group size */
        private int electableGroupSize;

        /* Cached value for ack group size */
        private int ackGroupSize;

        /*
         * Refreshes the cached summary values. This method must be invoked to
         * recompute the summary after any updates to the group.
         */
        private void refresh() {
            electableGroupSize = countAllMembers(ELECTABLE_PREDICATE);
            ackGroupSize = countAllMembers(ACK_PREDICATE);
        }
