...
public class RollbackException extends RestartRequiredException {
    ...
    private static String makeMessage
        ...
        final boolean durable = searchResults.getNumPassedDurableCommits() > 0;
        final String durableMsg = searchResults.getNumPassedCommits() > 0 ?
            (" Rolled back past " +
                (durable ? "durable" : "non-durable") +
                " transaction commit or abort.") :
            " Uncommited operations discarded" ;
        return "Node " + nodeName +
            ...
            Long.toHexString(DbLsn.getFileNumber(truncateLSN)) +
            ", offset 0x" +
            Long.toHexString(DbLsn.getFileOffset(truncateLSN)) +
            ", vlsn " + matchpointVLSN + "." + durableMsg;
    }
    ...
    public int getNumPassedDurableCommits() {
        return searchResults.getNumPassedDurableCommits();
    }
    ...
    public int getNumPassedCommits() {
        return searchResults.getNumPassedCommits();
    }

    public boolean rollbackDurableTxn() {
        if (searchResults == null) {
            return false;
        }
        return searchResults.getNumPassedDurableCommits() > 0;
    }

    /**
     * Only for use by wrapSelf methods.
     * Is protected for use by KVS tests.
     */
    protected RollbackException(String message, RollbackException cause) {
        super(message, cause);
    ...
    public RollbackException wrapSelf(
        ...
        return new RollbackException(msg, (RollbackException) clonedCause);
    }
}
