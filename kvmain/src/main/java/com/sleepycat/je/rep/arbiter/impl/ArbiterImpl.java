...
public class ArbiterImpl extends StoppableThread {
    ...
    public void runArbiter() {
        ...
        /*
         * Constructs the elections with default pre-promise and pre-learn hooks
         * instead of the ones with more work such as ElectionPrePromiseHook.
         *
         * Those specialized hooks are used to ensure that entries from lower
         * term cannot be acked after a promise of higher term is made such
         * that no durable entry is missing when a master is chosen. Such is
         * not a concern for arbiters. This is because the
         * RankingProposer#choosePhase2Value guarantees that if arbiter has the
         * highest vlsn, then no master is chosen.
         *
         * We can argue by contraction and induction. Suppose that an entry e is
         * majority-acked by an arbiter at term T1 for master M1; the arbiter
         * also loses the vote to M2 at term T2; M2 does not have e when it is
         * elected. If the arbiter loses the vote, then M2 has higher vlsn. Then
         * M2 must have e.
         */
        elections = new Elections(new ArbElectionsConfig(),
                                  new ElectionStates((byte) 0),
                                  suggestionGenerator,
                                  changeListener);
    ...
    public void setState(ReplicatedEnvironment.State state) {
        ...
        repImpl.getNodeState().arbiterChangeAndNotify(state);
    }
    ...
    void notifyJoinGroup() {
        this.joinGroupTime = TimeSupplier.currentTimeMillis();
    }
    ...
    private class MasterSuggestionGenerator
        ...
        public Ranking getRanking(Proposal proposal) {
            return new Ranking(arbiterVLSNTracker.getDTVLSN(),
                               arbiterVLSNTracker.get(),
                               arbiterVLSNTracker.getLastAckNodeId(),
                               arbiterVLSNTracker.getMasterTerm());
        }
    }
}
