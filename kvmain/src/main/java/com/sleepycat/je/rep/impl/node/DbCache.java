...
public class DbCache  {
     ...
     */
    private int tick = 1;
    private long tickTime = TimeSupplier.currentTimeMillis();
    ...
    public void tick() {
        ...
        if ((timeoutMs > 0) &&
            (TimeSupplier.currentTimeMillis() - tickTime) <= timeoutMs) {
            return;
        }
        ...
        tick++;
        tickTime = TimeSupplier.currentTimeMillis();
    }
    ...
    public DatabaseImpl get(DatabaseId dbId, Txn txn) {
        ...
        info = new Info(dbTree.getDb(dbId));
        map.put(dbId, info);
