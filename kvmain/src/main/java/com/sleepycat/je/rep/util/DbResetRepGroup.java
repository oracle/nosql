 ...
 * <a href="{@docRoot}/../ReplicationGuide/lifecycle.html#lifecycle-nodestartup"> replication node lifecycle</a>
...
public class DbResetRepGroup {
    ...
    private static final String usageString =
        ...
        "                                          for this node\n" +
        " -recover                              # use this flag to\n" +
        "                                       # recover and reset the\n" +
        "                                       # replication group, may\n" +
        "                                       # have to perform" +
        "                                       # NetworkRestore after reset.";
    ...
    private void parseArgs(String[] args) {
        ...
        while (argc < nArgs) {
            ...
            } else if (thisArg.equals("-recover")) {
                recover = true;
            }
        }
     ...
     * @param recover Recover the replication group database.
     */
     ...
     */
     public void reset() {
        ...
        if (recover) {
            /*
             * Set the RESET_REP_GROUP_RETAIN_UUID flag because the idea is to
             * keep the group information the same, instead of creating a new
             * group.  Usually the group record is too corrupted or missing,
             * so the UUID cannot be read, but there are cases where the group
             * record is fine, but one of the node records is corrupted, thus
             * preventing the replication group from starting up.
             */
            repConfig.setConfigParam(
                RepParams.RESET_REP_GROUP_RETAIN_UUID.getName(), "true");
            repConfig.setConfigParam(
                RepParams.RESET_REP_GROUP_RECOVER.getName(), "true");
        }
        ...
        /*
         * Clean the logs and do a checkpoint.  This prevents the joining
         * nodes from replaying the transaction that corrupted the RepGroupDB
         * in the first place, which can cause a fatal exception in the
         * joining node.
         */
        if (recover) {
            repEnv.cleanLog();
