...
import com.sleepycat.je.config.DurationConfigInMillisParam;
...
public class RepParams {
    ...
    /**
     * @hidden
     * @see ReplicationConfig#ASYNC_ACK_TIMER_PERIOD
     */
    public static final DurationConfigParam ASYNC_ACK_TIMER_PERIOD =
        new DurationConfigParam(ReplicationConfig.ASYNC_ACK_TIMER_PERIOD,
                                "100 ms",                    // min
                                null,                        // max
                                "1 s",                       // default
                                false,                       // mutable
     ...
     * TimeConsistencyPolicy(<i>permissibleLag</i>,<i>timeout</i>). For
    ...
    /* Commit-time lag tracking properties */
    ...
    public static final DurationConfigParam
        ...
        new DurationConfigParam(
            ReplicationConfig.TXN_END_TIME_LAG_TRACKING_UPDATE_INTERVAL,
            "1 s",     // min
            null,       // max
            "10 s",     // default
            false,      // mutable
    ...
    public static final DurationConfigParam
        ...
        new DurationConfigParam(
            ReplicationConfig.TXN_END_TIME_LAG_TRACKING_INITIAL_THRESHOLD,
            "1 ms",     // min
            null,       // max
            "10 s",     // default
            false,      // mutable
    ...
    /**
     * The duration of the short-term period. A histogram is kept with a
     * dampening factor adjusted to the short-term period. This short-term
     * histogram is used to compare with the recent histogram for anomaly
     * detection.
     */
    public static final LongDurationConfigParam
        TXN_END_TIME_LAG_TRACKING_SHORT_TERM_PERIOD =
        new LongDurationConfigParam(
            ReplicationConfig.TXN_END_TIME_LAG_TRACKING_SHORT_TERM_PERIOD,
            "10 s",     // min
            null,       // max
            "1 days",   // default 1 day
            false,      // mutable
            true);     // forReplication

    /**
     * The duration of the long-term period. A histogram is kept with a
     * dampening factor adjusted to the long-term period. This long-term
     * histogram is used to compare with the recent histogram for anomaly
     * detection.
     */
    public static final LongDurationConfigParam
        ...
        new LongDurationConfigParam(
            ReplicationConfig.TXN_END_TIME_LAG_TRACKING_LONG_TERM_PERIOD,
            "10 s",     // min
            null,       // max
    ...
    public static final EnumConfigParam<NodeType> NODE_TYPE =
        new EnumConfigParam<NodeType>(ReplicationConfig.NODE_TYPE,
                                      ...
                                      NodeType.class) {
        @Override
        public void validateValue(String nodeType)
            throws IllegalArgumentException {
            super.validateValue(nodeType);
            if (Enum.valueOf(NodeType.class, nodeType) == NodeType.MONITOR) {
                throw new IllegalArgumentException
                    ("MONITOR nodes are no longer supported");
            }
        }
    };
    ...
    /**
     * The time interval, in nanoseconds, during which transactions requiring
     * acknowledgment may be batched at the feeder before being written to the
     * network.
     * <p>
     * While it's possible to specify very small values, as a practical matter
     * values below 200ns are unlikely to be meaningful, since they are in the
     * range of times taken to schedule threads on most operating systems.
     * <p>
     * A value of zero means that records will not be batched and will be sent
     * out immediately as soon as they are available; this setting is typically
     * undesirable, due to its high overheads.
     * <p>
     * Larger values can result in fewer network packets and lower interrupt
     * processing overheads, but higher overall operation latency.
     * <p>
     ...
     * <p>
     * The batch size is also influenced by {@link #FEEDER_BATCH_BUFF_KB}, that
     * is, the feeder batch is written out if either the time threshold or the
     * storage threshold is breached.
     *
     */
    public static final IntConfigParam FEEDER_ACK_TXN_BATCH_NS =
        new IntConfigParam(EnvironmentParams.REP_PARAM_PREFIX +
                           "feederAckTxnBatchNs",
                           Integer.valueOf(0),                // min
                           Integer.valueOf(1_000_000_000),    // max 1 second
                           Integer.valueOf(1_000_000),        // default 1 ms
                           true,                              // mutable
                           true);                             // forReplication
     ...
     * initiated. The default value is 8K (9000 bytes - network overheads) to
     * take advantage of networks that support jumbo frames.
     * <p>
     * The batch size is also influenced by {@link #FEEDER_ACK_TXN_BATCH_NS}, that
     * is, the feeder batch is written out if either the time threshold or the
     * storage threshold is breached.
     */
    ...
    /**
     * @see ReplicationConfig#DISABLE_AUTHORITATIVE_MASTER
     */
    public static final BooleanConfigParam DISABLE_AUTHORITATIVE_MASTER =
        new BooleanConfigParam(ReplicationConfig.DISABLE_AUTHORITATIVE_MASTER,
                               false,            // default
                               true,             // mutable
                               true);            // forReplication

    /**
     * @see ReplicationConfig#DISABLE_OPENING_PORT
     */
    public static final BooleanConfigParam DISABLE_OPENING_PORT =
        new BooleanConfigParam(ReplicationConfig.DISABLE_OPENING_PORT,
                               false,            // default
                               false,            // mutable
                               true);            // forReplication

    /**
     * @see ReplicationMutableConfig#OVERRIDE_MIN_ELECTION_TIME
     */
    public static final IntConfigParam OVERRIDE_MIN_ELECTION_TIME =
        new IntConfigParam(ReplicationMutableConfig.OVERRIDE_MIN_ELECTION_TIME,
                               0,                             // min
                               Integer.MAX_VALUE,             // max
                               1000,                          // default
                               true,                          // mutable
    ...
    /**
     * An internal option, used with {@link #RESET_REP_GROUP} and
     * {#link {@link #RESET_REP_GROUP_RETAIN_UUID} to reset a replication group
     * even when the replication group database is unreadable.
     */
    public static final BooleanConfigParam RESET_REP_GROUP_RECOVER =
        new BooleanConfigParam(EnvironmentParams.REP_PARAM_PREFIX +
                               "resetRepGroupRecover",
                               false,            // default
                               false,            // mutable
                               true);
    ...
    /*
     * Sets election clock skew.  Only used in testing.
     */
    public static final DurationConfigParam ELECTIONS_CLOCK_SKEW =
        new DurationConfigParam(ReplicationConfig.TEST_ELECTIONS_CLOCK_SKEW,
                                null,               // min
                                "1 min",            // max
                                "0 s",              // default
                                false,              // mutable
    ...
    /*
     * Sets heartbeat clock skew.  Only used in testing.
     */
    public static final DurationConfigParam HEARTBEAT_CLOCK_SKEW =
        new DurationConfigParam(ReplicationConfig.TEST_HEARTBEAT_CLOCK_SKEW,
                                null,               // min
                                "1 min",            // max
                                "0 s",              // default
                                false,              // mutable
    ...
    public static final ConfigParam HELPER_HOSTS =
        new ConfigParam(ReplicationMutableConfig.HELPER_HOSTS,
                        "",                  // default
                        true,                // mutable
                        true) {              // forReplication
        ...
        public void validateValue(String hostPortPairs)
            ...
            HashSet<String> hostPortSet = new HashSet<>();
    ...
    /*
     * Whether optimizing for size of replicated entries is more important
     * than the extra memory allocation required to do so. Due to the priority
     * of lowering GC costs, it is unlikely that this param will need to be set
     * to true, which is why it is not exposed in the API.
     */
    static final BooleanConfigParam OPTIMIZE_STREAM_FOR_SIZE =
        new BooleanConfigParam
        (EnvironmentParams.REP_PARAM_PREFIX + "optimizeStreamForSize",
         false,             // default
         false,             // mutable
         true);             // forReplication

    /*
     * Whether to enable the replay preprocessor. We expect that enabling it
     * will always be beneficial, especially with the default number of
     * threads, which is why it is not exposed in the API.
     */
    public static final BooleanConfigParam REPLAY_PREPROCESSOR =
        new BooleanConfigParam
        (EnvironmentParams.REP_PARAM_PREFIX + "replayPreprocessor",
         true,              // default
         false,             // mutable
         true);             // forReplication

    /*
     * Number of threads (core and max) in the replay preprocessor pool. It is
     * unclear whether tuning this parameter will be needed, which is why it
     * is not exposed in the API.
     */
    public static final IntConfigParam REPLAY_PREPROCESSOR_THREADS =
        new IntConfigParam
        (EnvironmentParams.REP_PARAM_PREFIX + "replayPreprocessorThreads",
         1,      // min
         null,   // max
         2,      // default
         false,  // mutable
    ...
    /**
     * @see ReplicationConfig#TEST_ELECTIONS_MAX_PROPOSE_ATTEMPTS
     */
    public static final IntConfigParam TEST_ELECTIONS_MAX_PROPOSE_ATTEMPTS =
        new IntConfigParam(
            ReplicationConfig.TEST_ELECTIONS_MAX_PROPOSE_ATTEMPTS,
            0,
            Integer.MAX_VALUE,
            Integer.MAX_VALUE,
            false,
    ...
    public static final DurationConfigParam REPLAY_DB_HANDLE_TIMEOUT =
        new DurationConfigParam(ReplicationMutableConfig.
    ...
    /* @see ReplicationConfig#MASTER_MAX_GROUP_COMMIT */
    public static final IntConfigParam MASTER_MAX_GROUP_COMMIT =
        new IntConfigParam(ReplicationConfig.MASTER_MAX_GROUP_COMMIT,
                           Integer.valueOf(0),      // min
                           null,                      // max
                           Integer.valueOf(0),    // default
                           false,                     // mutable
    ...
    /* @see ReplicationConfig#MASTER_GROUP_COMMIT_INTERVAL */
    public static final DurationConfigParam MASTER_GROUP_COMMIT_INTERVAL =
        new DurationConfigParam(ReplicationConfig.MASTER_GROUP_COMMIT_INTERVAL,
                                "0 ms",      // min
                                null,               // max
                                "3 ms",             // default
                                false,              // mutable
    ...
    public static final DurationConfigInMillisParam PRE_HEARTBEAT_TIMEOUT =
        new DurationConfigInMillisParam
     ...
     * eviction.
     */
    static final BooleanConfigParam CACHE_RECORD_VERSION =
        new BooleanConfigParam(EnvironmentParams.REP_PARAM_PREFIX +
                               "cacheRecordVersion",
                               true,          // default
                               false,         // mutable
    ...
    /**
     * SSL need client authentication
     * @see ReplicationSSLConfig#SSL_NEED_CLIENT_AUTH
     */
    public static final ConfigParam SSL_NEED_CLIENT_AUTH =
        new ConfigParam(ReplicationSSLConfig.SSL_NEED_CLIENT_AUTH,
                        "false",             // default
                        false,               // mutable
                        true);               // forReplication
    ...
    /**
     * SSL TrustStore password source class
     * @see ReplicationSSLConfig#SSL_TRUSTSTORE_PASSWORD_CLASS
     */
    public static final ConfigParam SSL_TRUSTSTORE_PASSWORD_CLASS =
        new ConfigParam(ReplicationSSLConfig.SSL_TRUSTSTORE_PASSWORD_CLASS,
                        "",                  // default
                        false,               // mutable
                        true);               // forReplication

    /**
     * SSL TrustStore password source constructor parameters
     * @see ReplicationSSLConfig#SSL_TRUSTSTORE_PASSWORD_PARAMS
     */
    public static final ConfigParam SSL_TRUSTSTORE_PASSWORD_PARAMS =
        new ConfigParam(ReplicationSSLConfig.SSL_TRUSTSTORE_PASSWORD_PARAMS,
                        "",                  // default
                        false,               // mutable
                        true);               // forReplication
