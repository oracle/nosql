...
public class LDiff {
    ...
    private boolean diff(Database db, DataChannel channel)
        ...
        while (true) {
            try {
                ...
                if (pe.getUnexpectedMessage().getOp().getMessageClass() !=
                    Protocol.BLOCK_LIST_END_CLASS) {
                    throw pe;
                }
                break;
            }
    ...
    public boolean diff(Database db, BlockBag blkBag)
        ...
        while (window.getChecksum() != 0 && blkBag.size() > 0) {
            ...
            /*
             * Find the block in the bag whose checksum and sha256 match the
    ...
    /**
     * Find the block in the bag whose rolling checksum and sha256 match the
     * given window. The sha256 for the window is computed lazily, since it's
     * more expensive to compute. There is a slight chance that multiple blocks
     * in the bag will match the checksum and the sha256 hash, return the first
     * such block added to the bag in that case.
     *
     * @param blkBag a bag of blocks to search for a match
     * @param window the block sized window of the db we're diffing
     * @return A block which matches the window's checksum and the window's
     * sha256 hash, or null if no block matches.
     */
    private Block findMatch(Environment env, BlockBag blkBag, Window window) {
        ...
        byte[] sha256 = window.getSHA256Hash();
        for (Block b : matches) {
            if (Arrays.equals(b.getSHA256Hash(), sha256)) {
                return b;
            }
            LoggerUtils.envLogMsg
                (Level.FINE,
                 ...
                 "matches LB but sha-256 hash doesn't:" + b);
        }
        /* No matches. */
        return null;
    }
    ...
    public BlockBag createBlockBag(Database db) {
        ...
        long end = TimeSupplier.currentTimeMillis();
        LoggerUtils.envLogMsg
            (Level.FINE,
             DbInternal.getNonNullEnvImpl(db.getEnvironment()),
    ...
    class MismatchException extends Exception implements NotSerializable {
    ...
    private class LDiffIterator implements Iterator<Block> {
        ...
        public Block next() {
            ...
            Block ret = cached;
            cached = LDiffUtil.readBlock(
                i++, cursor, lastKey, lastData, numKeys);
