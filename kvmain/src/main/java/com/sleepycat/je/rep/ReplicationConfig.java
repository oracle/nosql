 ...
 *
 * <h2>Getting the Current ReplicatedEnvironment Properties</h2>
 *
...
public class ReplicationConfig extends ReplicationMutableConfig {
    ...
    /**
     * Used to disable delaying write and absolute consistent transactions
     * until enough replicas confirm that the master is still the master.
     * Should only be used for performance testing as this fixes a bug
     * that exists when there is a network partition and a duplicate master
     * is elected.
     *
     * <table border="1">
     * <caption style="display:none">Information about configuration option</caption>
     ...
     */
    public static final String DISABLE_AUTHORITATIVE_MASTER =
        EnvironmentParams.REP_PARAM_PREFIX + "disableAuthoritativeMaster";

    /**
     * Used to disable opening tcp/ip port.
     * Should only be used internally by server code.
     *
     * <table border="1">
     * <caption style="display:none">Information about configuration option</caption>
     * <tr>
     ...
     * </table>
     */
    public static final String DISABLE_OPENING_PORT =
        EnvironmentParams.REP_PARAM_PREFIX + "disableOpeningPort";
     ...
     * where {@code <permissibleLag>} and {@code <timeout>} are <a
     ...
     * @hidden
     *
     * The interval to collect commit-time lag histogram to detect anomaly and
     * update the stats. Recent lag values are kept in a histogram is kept for
     * the last period of this specified time. When the period passes, the
     * distance is computed between the recent histogram and the past ones for
     * anomaly detection. The past histograms are then updated with the recent
     * one and the anomaly detection stats are updated to the stats mechanism.
     */
    public static final String TXN_END_TIME_LAG_TRACKING_UPDATE_INTERVAL =
        EnvironmentParams.REP_PARAM_PREFIX
        + "txnEndTimeLagTrackingUpdateInterval";

    /**
     * @hidden
     *
     * The initial commit-time lag threshold. A message will be logged when the
     * threshold is passed after a replica re-connect to the master.
     */
    public static final String TXN_END_TIME_LAG_TRACKING_INITIAL_THRESHOLD =
        EnvironmentParams.REP_PARAM_PREFIX
        + "txnEndTimeLagTrackingInitialThreshold";

    /**
     * @hidden
     *
     * The duration of the short-term period for commit-time lag anomaly
     * detection. A histogram is kept with a dampening factor adjusted to the
     * specified period such that lag values outside of this period have much
     * smaller impact. The histogram will be used to compute a distance with
     * the most recent histogram. This value should be set to represent the
     * duration of short-term workload changes.
     */
    public static final String TXN_END_TIME_LAG_TRACKING_SHORT_TERM_PERIOD =
        EnvironmentParams.REP_PARAM_PREFIX
        + "txnEndTimeLagTrackingShortTermPeriod";

    /**
     * @hidden
     *
     * The duration of the long-term period for commit-time lag anomaly
     * detection. A histogram is kept with a dampening factor adjusted to the
     * specified period such that lag values outside of this period have much
     * smaller impact. The histogram will be used to compute a distance with
     * the most recent histogram. This value should be set to represent the
     * duration of long-term workload changes.
     */
    public static final String TXN_END_TIME_LAG_TRACKING_LONG_TERM_PERIOD =
        EnvironmentParams.REP_PARAM_PREFIX
        + "txnEndTimeLagTrackingLongTermPeriod";
    ...
    /**
     * The async ack timeout task runs with this period, thus determining the
     * precision of the async ack timeout. The period must be expressed in
     * milliseconds.
     *
     * @since 23.1
     */
    public static final String ASYNC_ACK_TIMER_PERIOD =
        EnvironmentParams.REP_PARAM_PREFIX + "asyncAckTimerPeriod";
    ...
    /**
     * Sets the maximum acceptable clock skew between any two nodes. This value
     * is checked whenever two nodes communicate, either for master election or
     * replication. The environment is invalidated if the clock skew is larger
     * than this value. The cause (e.g., NTP configuration needs adjustment,
     * the clock hardware is broken, etc.) should be investigated before
     * restarting the environment.
     *
     * <table border="1">
     * <caption style="display:none">Information about configuration
     * option</caption>
     * <tr>
     * <td>Name</td>
     * <td>Type</td>
     ...
     * <tr>
     * <td>{@value}</td>
     * <td><a href="../EnvironmentConfig.html#timeDuration">Duration</a></td>
     * <td>No</td>
     * <td>2 s</td>
     * <td>0 s</td>
     * <td>1 min</td>
    ...
    /**
     * @hidden
     * Sets a clock skew during elections.  This should only be used in testing.
     *
     * <table border="1">
     * <caption style="display:none">Information about configuration option</caption>
     * <tr><td>Name</td><td>Type</td><td>Mutable</td>
     ...
     * </table>
     *
     * @see ReplicationConfig#setElectionClockSkew
     * @see ReplicationConfig#getElectionClockSkew
     * @see <a href="../EnvironmentConfig.html#timeDuration">Time Duration
     * Properties</a>
     */
    public static final String TEST_ELECTIONS_CLOCK_SKEW =
        EnvironmentParams.REP_PARAM_PREFIX + "electionClockSkew";

    /**
     * @hidden
     *
     * Sets a clock skew for heartbeat and response. This should only be used
     * in testing.
     *
     * <table border="1">
     * <caption style="display:none">Information about configuration
     ...
     * </table>
     *
     * @see ReplicationConfig#setHeartbeatClockSkew
     * @see ReplicationConfig#getHeartbeatClockSkew
     * @see <a href="../EnvironmentConfig.html#timeDuration">Time Duration
     * Properties</a>
     *
     */
    public static final String TEST_HEARTBEAT_CLOCK_SKEW =
        EnvironmentParams.REP_PARAM_PREFIX + "heartbeatClockSkew";

    /**
     * The number of times a proposer attempts an election. This is currently
     * only used for testing.
     *
     * <p>
     * During election, the proposer of this node issues proposals to make
     * progress until a master is learned. This parameter controls the number
     * of total attempts for the proposer before it stops issuing proposals in
     * the case of a master cannot be established.  The environment will be
     * invalidated in this case when the maximum is exceeded.  The usual
     * culprit in this case is a network issue, e.g., hostnames and ports are
     * misconfigured.
     *
     * <p>
     * This parameter is only used in test. In production, we set this maximum
     * to Integer.MAX_VALUE. With at least one second backoff time, it will
     * take at least 25 days before the environment is invalidated. This
     * behavior is designed with the principle of make HA as the highest
     * priority. Exceptions are logged in various places to draw the attention
     * of the users, but the system just waits and retries in case the problem
     * can resolve itself.
     *
     * <table border="1">
     * <caption style="display:none">Information about configuration
     ...
     * </table>
     */
    public static final String TEST_ELECTIONS_MAX_PROPOSE_ATTEMPTS =
        EnvironmentParams.REP_PARAM_PREFIX + "electionsMaxProposeAttempts";
     ...
     * The maximum number of transactions that can be grouped to amortize the
     * cost of an fsync when a transaction commits with SyncPolicy#SYNC on the
     * Master. A value of zero effectively turns off the group commit
     * optimization.
     * <p>
     * Specifying larger values can result in more transactions being grouped
     * together decreasing average commit times.
     ...
     * An fsync is issued if the size of the transaction group reaches the
     * maximum within the time period specified by
     * {@link #MASTER_GROUP_COMMIT_INTERVAL}.
     * <p>
     * The {@link
     * ReplicatedEnvironmentStats#getNMasterGroupCommitMaxExceeded()}
     * statistic may be used to tune this parameter. Large values indicate that
     * commit throughput could be improved by increasing the current value.
     * <table border="1">
     * <caption style="display:none">Information about configuration option</caption>
     ...
     *
     * @since 25.1.4
     * @see #MASTER_GROUP_COMMIT_INTERVAL
     */
    public static final String MASTER_MAX_GROUP_COMMIT =
        EnvironmentParams.REP_PARAM_PREFIX + "masterMaxGroupCommit";

    /**
     * The time interval during which transactions may be grouped to amortize
     * the cost of fsync when a transaction commits with SyncPolicy#SYNC on the
     * Replica. This parameter is only meaningful if the
     * {@link #MASTER_MAX_GROUP_COMMIT group commit size} is greater than one.
     * <p>
     * The leader transaction in a transaction group may be delayed by at most
     * this amount. Subsequent transactions in the group will return immediately
     * if the size of the group doesn't exceed {@link #MASTER_MAX_GROUP_COMMIT group commit size}.
     * Leader transaction will issue a fsync after the configured interval,
     * if no other transaction issues one.
     * <p>
     * The {@link ReplicatedEnvironmentStats#getNMasterGroupCommitTimeouts()}
     * statistic may be used to tune this parameter. Large numbers of timeouts
     * in conjunction with large numbers of group commits (
     * {@link ReplicatedEnvironmentStats#getNMasterGroupCommits()}) indicate
     * that commit throughput could be improved by increasing the time
     * interval.
     * <table border="1">
     * <caption style="display:none">Information about configuration option</caption>
     ...
     *
     * @since 25.1.4
     * @see #MASTER_MAX_GROUP_COMMIT
     */
    public static final String MASTER_GROUP_COMMIT_INTERVAL =
        EnvironmentParams.REP_PARAM_PREFIX + "masterGroupCommitInterval";
    ...
    /**
     * Disables delaying transactions until it is confirmed that no other
     * master has been elected and is accepting new transactions.
     *
     * @see #DISABLE_AUTHORITATIVE_MASTER
     * @return this
     */
    public boolean getDisableAuthoritativeMaster() {
        return DbConfigManager.
            getBooleanVal(props, RepParams.DISABLE_AUTHORITATIVE_MASTER);
    }

    /**
     * Sets whether transactions will be delayed in replication until it is
     * confirmed that no other master has been elected and is accepting
     * new transactions.
     *
     * @see #DISABLE_AUTHORITATIVE_MASTER
     *
     ...
     * @return this
     */
    public ReplicationConfig setDisableAuthoritativeMaster(boolean override) {

        DbConfigManager.
            setBooleanVal(props, RepParams.DISABLE_AUTHORITATIVE_MASTER,
                          override, validateParams);
        return this;
    }

    /**
     * Disables opening tcp/ip ports.
     *
     * @see #DISABLE_OPENING_PORT
     * @return this
     */
    public boolean getDisableOpeningPort() {
        return DbConfigManager.
            getBooleanVal(props, RepParams.DISABLE_OPENING_PORT);
    }

    /**
     * Sets whether tcp/ip ports will be opened.
     *
     * @see #DISABLE_OPENING_PORT
     *
     * @param override whether to disable opening ports.
     * @return this
     */
    public ReplicationConfig setDisableOpeningPort(boolean override) {

        DbConfigManager.
            setBooleanVal(props, RepParams.DISABLE_OPENING_PORT,
                          override, validateParams);
        return this;
    }
    ...
    /**
     * @hidden
     * Gets the clock skew in elections.  Used for testing only.
     */
    public long getElectionClockSkew(TimeUnit unit) {
        return DbConfigManager.getDurationVal(props,
                                              RepParams.ELECTIONS_CLOCK_SKEW,
                                              unit);
    }

    /**
     * @hidden
     * Sets the clock skew in elections.  Used for testing only.
     */
    public ReplicationConfig setElectionClockSkew(long skew,
                                              TimeUnit unit)
        throws IllegalArgumentException {

        DbConfigManager.setDurationVal(props, RepParams.ELECTIONS_CLOCK_SKEW,
                                       skew, unit, validateParams);
        return this;
    }

    /**
     * @hidden
     * Gets the clock skew in heartbeat.  Used for testing only.
     */
    public long getHeartbeatClockSkew(TimeUnit unit) {
        return DbConfigManager.getDurationVal(props,
                                              RepParams.HEARTBEAT_CLOCK_SKEW,
                                              unit);
    }

    /**
     * @hidden
     * Sets the clock skew in heartbeat.  Used for testing only.
     */
    public ReplicationConfig setHeartbeatClockSkew(long skew,
                                                   TimeUnit unit)
        throws IllegalArgumentException {

        DbConfigManager.setDurationVal(props, RepParams.HEARTBEAT_CLOCK_SKEW,
                                       skew, unit, validateParams);
        return this;
    }
