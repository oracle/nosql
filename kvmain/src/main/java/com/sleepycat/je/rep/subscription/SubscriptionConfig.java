...
public class SubscriptionConfig implements Cloneable {
    ...
    /* The any host, any port socket address. */
    public final static InetSocketAddress ANY_ADDRESS =
    ...
    public String getFeederHostPort() {
        return feederHostPortPair;
    }
    ...
    private class DefaultCallback implements SubscriptionCallback {
        ...
        @Override
        public void processDel(long vlsn, byte[] key, byte[] value, long txnId,
                               DatabaseId dbId, long ts,
                               boolean beforeImgEnabled,
                               byte[] valBeforeImg,
                               long tsBeforeImg,
                               long expBeforeImg) {
     ...
     */
    public static class DefaultFeederFilter
            implements FeederFilter, Serializable {
        private static final long serialVersionUID = 1L;

        /*
         * Remember the internal and dup db ids that the filter has seen,
         * it does not need to call getDb again for new entry. The db id is
         * immutable.
         */
        final Set<DatabaseId> cachedIntDupDBId;

        /* Used to obtain LN info from wire record. */
        protected transient LNEntryInfo lnInfo;

        protected DefaultFeederFilter() {
            super();
            cachedIntDupDBId = new HashSet<>();
        }
        ...
        public OutputWireRecord execute(final OutputWireRecord record,
            ...
            /* Create transient lnInfo on-demand after deserialization. */
            if (lnInfo == null) {
            ...
            /* known internal or dup db id */
            if (cachedIntDupDBId.contains(dbId)) {

                /* remove dup db id from cache if deleted */
                final byte type = record.getEntryType();
                if (LogEntryType.LOG_NAMELN.equalsType(type) ||
                    LogEntryType.LOG_NAMELN_TRANSACTIONAL.equalsType(type)) {
                    final NameLNLogEntry entry =
                        (NameLNLogEntry) record.instantiateEntry();
                    if (entry.getOperationType() == DbOperationType.REMOVE) {
                        final NameLN nameLn = (NameLN) entry.getMainItem();
                        cachedIntDupDBId.remove(nameLn.getId());
                    }
                }

                /* block the dup internal db entry */
                return null;
            }
