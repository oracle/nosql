 ...
 * <p>
 * ReplicatedEnvironment handles are analogous to {@link Environment} handles.
 * A replicated environment handle is a ReplicatedEnvironment instance. Only
 * one ReplicatedEnvironment instance may be created for a given physical
 * directory. In other words, only one ReplicatedEnvironment handle may be open
 * at a time for a given environment.
 * </p>
 * <p>
 ...
 * <a href="{@docRoot}/../ReplicationGuide/lifecycle.html">life
 ...
 * value. Membership information for electable nodes is stored in
 ...
 * <a href="{@docRoot}/../ReplicationGuide/progoverview.html#configrepenv">
 ...
 * Environment} and ReplicatedEnvironment are are not concurrent.
 * <p>
 ...
 * {@link com.sleepycat.je.Cursor} (when using the Base API) should be created,
 * used and closed before calling {@link ReplicatedEnvironment#close}.
 *
 ...
 * the group.
 *
 * <h2><a id="nonRepDbs">Non-replicated Databases in a Replicated
 * Environment</a></h2>
 *
 * A database in a replicated environment is replicated by default, but may be
 * explicitly configured as non-replicated using
 * {@link com.sleepycat.je.DatabaseConfig#setReplicated}. Such
 * non-replicated databases may be transactional or non-transactional.
 * The special considerations for using non-replicated databases in a
 * replicated environment are described below.
 * <p>
...
public class ReplicatedEnvironment extends Environment {
     ...
     * not been enabled for replication, or a handle for this environment is
     * already open.
     *
     ...
     * <p>
     * This method is called during construction but after opening of the
     * RepImpl. It closes the RepImpl abnormally if there is an exception.
     */
    ...
    /**
     * Ensure that the DataChannelFactory configuration is usable and
     * initialize logging state.
     * <p>
     * This method is called during construction but after opening of the
     * RepImpl. It closes the RepImpl abnormally if there is an exception.
     */
    private void initChannelFactory(RepImpl repImp) {
        try {
            repEnvironmentImpl.initializeChannelFactory();
        } catch (Throwable e) {
            repEnvironmentImpl.abnormalClose();
            throw e;
        }
    }
     ...
     * @hidden
     *
     * Note that repImpl.joinGroup is a synchronized method, and therefore
     * protected against multiple concurrent attempts to create a handle.
     *
    ...
    protected ReplicatedEnvironment(File envHome,
        ...
        /* Save resolved standalone config in case joinGroup fails below. */
        final EnvironmentConfig useEnvConfig = repEnvironmentImpl.getConfig();

        initChannelFactory(repEnvironmentImpl);
        ...
        if (joinGroup) {
            ...
            /* Replicated env initialization is now fully complete. */
            repEnvironmentImpl.fullyInitialized();
        } else {
            /* For testing only */
     ...
     */
    @Override
    protected ReplicationConfig setupRepConfig(File envHome,
                                               ReplicationConfig repConfig,
                                               EnvironmentConfig envConfig) {
     ...
     * <p>
     * Although the second wait period is brief, note that the transaction
     * timeout for blocked transactions is not honored during this period.
     * Aborting timed-out transactions would prolong the period during which
     * other transactions are blocked and could prevent completion of the
     * master transfer.
     *
     ...
     * replica.
     *
     * <li>The {@link #getState() state} of the old master and new master will
     * change to {@link State#isReplica()} and {@link State#isMaster()}
     * respectively in an asynchronous manner. The caller must not rely on the
     * state changes to have taken place immediately when this method returns.
     * </ol>
     *
     ...
     *
     * @param reason a description of why the master transfer is being
     * performed.
     *
	...
	public String transferMaster(Set<String> replicas,
								 ...
								 TimeUnit timeUnit,
								 String reason) {
		return transferMaster(replicas, timeout, timeUnit, false, reason);
	}
     ...
     * supplied in the argument list.
     *
     * @param force true if this request should supersede and cancel any currently
     *              pending Master Transfer operation
     *
     * @see #transferMaster(Set, int, TimeUnit)
     */
    public String transferMaster(Set<String> replicas,
                                 ...
                                 TimeUnit timeUnit,
                                 boolean force) {
        return transferMaster(replicas, timeout, timeUnit, force, null);
    }
