...
import com.sleepycat.je.rep.stream.Protocol.CacheSizeRequest;
...
public class FeederReplicaHandshake {
    ...
    public Protocol execute()
        ...
        checkCacheSize(protocol);
        LoggerUtils.info
            (logger, repNode.getRepImpl(),
             "Feeder-replica " + replicaNameIdPair.getName() +
             " handshake completed." +
             versionMsg +
             " Feeder Log version: " + LogEntryType.LOG_VERSION);
     ...
     * a warning on the replica side.  Mismatching cache sizes is not an error,
     * but can lead to a lagging replica if its cache size is not large enough
     * to efficiently handle the workload.
     */
    private void checkCacheSize(Protocol protocol)
        throws IOException,
               ProtocolException {
        /*
         * The cache size check was added in version 10, ignore if the replica
         * does not support version 10 or greater.
         */
        if (protocol.getVersion() < Protocol.VERSION_10) {
            return;
        }
        CacheSizeRequest info = protocol.read(
            namedChannel.getChannel(), CacheSizeRequest.class);
        writeMessage(protocol,
            protocol.new CacheSizeResponse(repNode.getRepImpl()));

        final RepImpl repImpl = repNode.getRepImpl();
        final long feederCacheSize = repImpl.getMemoryBudget().getMaxMemory();

        if (info.getCacheSize() != feederCacheSize
            || info.getOffHeapSize() != 0) {
            LoggerUtils.logMsg(logger, repImpl, Level.WARNING,
                "Mismatched cache sizes, feeder:" + feederCacheSize
                + " replica: " + info.getCacheSize()
                + " feeder off-heap: "+ 0
                + " replica off-heap: " + info.getOffHeapSize());
        }
    }
    ...
    private void maybeUpdateJEVersion(final NodeGroupInfo nodeGroup,
        ...
        if ((group.getFormatVersion() >= RepGroupImpl.FORMAT_VERSION_3) &&
            ...
            node.updateJEVersion(nodeGroup.getJEVersion());
             ...
             */
            try {
                repNode.getRepGroupDB().updateMember(node, false);
            } catch (InsufficientReplicasException |
                     InsufficientAcksException |
                     LockTimeoutException e) {
                /* Ignored */
    ...
    private Protocol negotiateProtocol()
        ...
        Protocol defaultProtocol =
            Protocol.getProtocol(repNode, namedChannel.getNameIdPair(),
        ...
        LoggerUtils.info(logger, repNode.getRepImpl(),
            "Replica:" + replicaNameIdPair +
            " requested protocol version:" + replicaVersion);
        Protocol protocol = Protocol.get(repNode, message.getNameIdPair(),
            replicaVersion, getCurrentProtocolVersion());
        if (protocol == null) {
            protocol = defaultProtocol;
        }
