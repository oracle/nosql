...
public interface SecondaryAssociation {
    ...
    /*
     * Returns the secondary databases that are associated with the given
     * primary key and need to be updated during the execution of an SQL
     * UPDATE statement. The allIndexDbNames, allIndexIds, and indexesToUpdate
     * are used to determine what indexes to update: see comment for these 3
     * fields in WriteOptions. However, if expirationUpdated or isTombstone is
     * true, all indexes need to be updated.
     *
     * @throws RuntimeException if an unexpected problem occurs.  The exception
     * will be thrown to the application, which should then take appropriate
     * action.
     */
    default Collection<SecondaryDatabase> getSecondaries(
        ...
        boolean isTombstone) {
        return getSecondaries(primaryKey);
    }
}
