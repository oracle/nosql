...
public class LogParams {
    ...
    /**
     * Set to true to call {@link LogItem#incrementUse()} before adding the
     * item the tip cache, to ensure that the item is not recycled when it is
     * removed from the tip cache. See ArbiterFeederSource.addCommit.
     */
    public boolean incLogItemUsage = false;

    /**
     * Set to true if 'entry' is immutable (in effect) and therefore may be
     * cached beyond the scope of the logging operation. If true, causes
     * LogItem.cachedEntry to be set initially.
     */
    public boolean immutableLogEntry = false;
