...
public abstract class FileReader {
     ...
     * EnvironmentFailureException and invalidating the environment.  This
        ...
        try {
            while ((!eof) && (!foundEntry)) {
                ...
                if (currentEntryHeader.isVariableLength()) {
                    ...
                    currentEntryHeader.readVariablePortion(
                        envImpl, dataBuffer);
                }
                 ...
                 * In rare cases it is possible for the log to be erased while
                 * it is being read into memory.
                 */
            ...
            /* Expected behavior logged elsewhere. */
            eof = true;
            throw e;
     ...
     * @throws ErasedException if the log was erased while being read into
     *  memory.
     */
