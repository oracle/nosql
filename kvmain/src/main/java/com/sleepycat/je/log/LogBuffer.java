...
public class LogBuffer implements LogSource {
    ...
    /*
     * The highestVLSN among VLSNs of log entries reside inside this
     * LogBufferSegment. Initialized to be -1, notice that it is possible
     * that no entry has an associated vlsn.
     */
     ...
     */
    LogBuffer(ByteBuffer buffer, long firstLsn, long vlsn) {
        this.buffer = buffer;
        this.firstLsn = firstLsn;
        this.lastLsn = firstLsn;
        this.highestVLSN = vlsn;
    }
     ...
     *
     * <p>numBytes is unused since buffer should always hold a whole entry.</p>
     *
     * <p>localBuffer is unused because a slice of the log buffer can be
     * returned.</p>
     *
     * The LWL or readLatch must be held.
     *
    ...
    public void setHighestVLSN(long vlsn) {
        if (vlsn >= highestVLSN &&
            vlsn != VLSN.INVALID_VLSN &&
            vlsn != VLSN.UNINITIALIZED_VLSN
           ) {
            /*
             * Will there be a chance that vlsn <= highestVLSN?
             */
            highestVLSN = vlsn;
        }
    }
