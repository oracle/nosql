...
public class ChildReference implements Loggable {
    ...
    public Node fetchTarget(DatabaseImpl database, IN parent)
        ...
        if (target == null) {
                ...
                if (!isKnownDeleted()) {
                    throw EnvironmentFailureException.unexpectedState(
                        IN.makeFetchErrorMsg(
                            envImpl, "fetchRoot", "NULL_LSN without KnownDeleted",
                            parent, null, lsn, state, 0));
                }
            ...
            try {
                 ...
                 * ChildReference is only used for RootChildReference, which
                 * never fetches an LN.
                 *
                 * The lastLoggedParam is -1 because we don't store the size
                 * of the root IN. Luckily, fetching it is very infrequent.
                 */
                WholeEntry wholeEntry = envImpl.getLogManager()
                    .getWholeLogEntryHandleNotFound(lsn, -1);
                Node node = (Node) wholeEntry.getEntry().getMainItem();
                ...
                in.postFetchInit(database, lsn, wholeEntry);
                target = node;
    ...
    public void readFromLog(EnvironmentImpl envImpl,
        ...
        lsn = LogUtils.readPackedLong(itemBuffer);           // LSN
