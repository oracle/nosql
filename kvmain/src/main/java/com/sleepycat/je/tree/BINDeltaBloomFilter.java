...
public class BINDeltaBloomFilter {
    ...
    public static class HashContext {
        ...
        void hashKeyPrefix(byte[] prefix) {
            initFNVvalue = BINDeltaBloomFilter.hashFNV(
                prefix, 0, prefix.length, initFNVvalue);
        }
    }
    ...
    /* Hack to get rid of Eclipse error */
    private static boolean assertKis3(int k) {
        return k == 3;
    }
     ...
     */
    private static void hash(byte[] bf,
                             byte[] key,
                             int keyOff,
                             int keyLen,
                             HashContext hc) {
        ...
        hc.rng.setSeed(hashFNV(key, keyOff, keyLen, hc.initFNVvalue));
     ...
     */
    private static long hashFNV(byte[] key,
                                int keyOff,
                                int keyLen,
                                long initValue) {
     ...
     * given log buffer.
     */
    public static byte[] readFromLog(ByteBuffer buffer) {
        return LogUtils.readByteArray(buffer);
    }
