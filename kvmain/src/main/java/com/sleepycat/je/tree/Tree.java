...
public final class Tree implements Loggable {
    ...
    /*
     * Splits without logging are disabled for now. It needs until debugged and
     * tested.
     */
    public static final boolean LOG_SPLITS = true;
    ...
    public void delete(byte[] idKey)
        ...
        try {
            ...
            /* Ensure the parent is flushed by an in-progress checkpoint. */
            envImpl.getCheckpointer()
                .coordinatePruningWithCheckpoint(branchParent);

            /*
             * If the subtree corresponding to the identifier key was removed,
             * then update the identifier key.
             */
            if (detachPoint.index == 0) {
                assert branchParent.isIN();
                branchParent.setIdentifierKey(branchParent.getKey(0), true);
            }
    ...
    public BIN getFirstNode(CacheMode cacheMode)
        ...
        if (bin != null) {
            bin.mutateToFullBIN(false /*leaveFreeSlot*/, cacheMode);
        }
     ...
     * node is latched and the argument BIN latch is released.
     */
    ...
    public IN getNextIN(
        ...
        prevIn.mutateToFullBIN(false /*leaveFreeSlot*/, cacheMode);
        ...
        try {
            while (true) {
                ...
                /*
                 * Note that curr is temporarily unlatched by latchParent and
                 * splits, etc, can occur in that window.
                 */
                parent = curr.latchParent();
                curr.releaseLatch();
    ...
    /**
     * Search for the parent of the given node.
     *
     * WARNING: This method is only reliable for finding the parent in the
     * following two cases:
     * 1. When there is no concurrent access to the Btree. This is the case for
     *    recovery.
     * 2. When it is acceptable for the wrong parent to be returned if the
     *    given child node is updated by concurrent activity. This is the case
     *    for cleaning, erasure and checkpointing, because if the child node is
     *    modified then it does not need to be explicitly cleaned, erased, or
     *    flushed by the checkpointer.
     * In other cases the wrong parent may be returned because of updates to
     * keys in UIN slots caused by key deletions. In such cases, the
     * IN.latchParent method should be used instead.
     *
     ...
     *
     * The method uses C's zero'th slot key to search, or the identifierKey if
     * no slots are present. We use the zero'th slot key (when available)
     * because we would like to phase out the use of the identifier key, since
     * it is not updated immediately to reflect changes in the IN slots.
     *
     * The method searches down the tree until:
     *
    ...
    public SearchResult getParentINForChildIN(
        ...
        byte[] targetKey = (child.getNEntries() > 0) ?
            child.getKey(0) : child.getIdentifierKey();
     ...
     * essentially overwriting the slot that may exist in the full BIN. So,
        ...
        try {
            ...
            while (true) {
                ...
                if (!rootExists()) {
                    ...
                    int maxTreeEntriesPerNode = database.getNodeMaxTreeEntries();
                     ...
                     */
                    IN rootIN =
                        new IN(database, key, -1, maxTreeEntriesPerNode, 2);
                    rootIN.setIsRoot(true);

                    rootIN.latch(cacheMode);

                    /* First BIN in the tree, log provisionally right away. */
                    bin = new BIN(database, key, -1, maxTreeEntriesPerNode, 1);
                    bin.latch(cacheMode);
                    LogItem binLogItem = bin.logProvisionalNoCompress(rootIN);
                    logLsn = binLogItem.lsn;
                     ...
                     */
                    int inserted = rootIN.insertEntry1(
                        bin, key, null, logLsn, false);
                    assert (inserted & IN.INSERT_SUCCESS) != 0;

                    rootIN.setLastLoggedSize(
                        inserted & ~IN.INSERT_SUCCESS, binLogItem.size);

                    logLsn = rootIN.log().lsn;
            ...
            do {
                ...
                if (child.needsSplitting()) {
                    ...
                    if (child.needsSplitting()) {
                         ...
                         */
                        BIN binResult = LOG_SPLITS ?
                            forceSplitLog(key, cacheMode, comparator) :
                            forceSplitNoLog(key, cacheMode, comparator);

                        assert binResult.isLatchOwner();

                        success = true;
                        return binResult;
                    }
                } else if (child.getNEntries() != 0) {

                    /*
                     * Check if the key being inserted is less than current
                     * 0th index key.
                     */
                    int cmp = child.entryKeys.compareKeys(
                        key, child.keyPrefix, 0,
                        child.haveEmbeddedData(0),
                        database.getKeyComparator());

                    if (cmp < 0) {
                        /*
                         * 0th key in the child node is not the smallest key
                         * in the subtree represented by the 0th key. Update
                         * the child node to reflect that.
                         */
                        child.setFlagSubtreeSlotsReflectLeastValue(false);
                    }
                }
     ...
     */
    private BIN forceSplitLog(byte[] key,
                              CacheMode cacheMode,
                              InternalComparator comparator)
        ...
        try {
            ...
            for (int i = nodeLadder.size() - 1; i >= 0; i -= 1) {
                ...
                if (child.needsSplitting()) {
                    ...
                    if (i > 0) {
                        final SplitInfo prevInfo = nodeLadder.get(i - 1);
                        grandParent = prevInfo.parent;
                        parentIndex = prevInfo.index;
                    } else {
                        grandParent = null;
                        parentIndex = -1;
                    }
                    ...
                    if (allLeftSideDescent || allRightSideDescent) {
                        newSibling = child.splitSpecial(
                            parent, index, grandParent, parentIndex,
                            maxTreeEntriesPerNode, key, allLeftSideDescent);
                    } else {
                        newSibling = child.split(
                            parent, index, grandParent, parentIndex,
                            maxTreeEntriesPerNode);
                    }
                    info.newChild = newSibling;

                    if (child.isBIN()) {
                        binResult = findSplitBranch(
                            parent, key, comparator, child, newSibling);
                    }
                     ...
                     */
                    if (parent.isRoot()) {
                        root.setLsn(parent.getLastLoggedLsn());
                    }
                } else {
                    if (startedSplits) {
                        /*
                        ...
                        if (lastParentForSplit != child.getNodeId()) {
                            final LogItem newChildItem =
                                child.logProvisional(parent);

                            parent.updateEntry(
                                index, newChildItem.lsn, NULL_VLSN,
                                newChildItem.size);
                        }
                         ...
                         */
                        if (parent.isRoot()) {
                            final long newRootLsn = parent.log().lsn;
                            root.setLsn(newRootLsn);
                        }
                    }
                }
            }
            ...
            /*
             * Release all latches except for binResult's latch.
             */
            rootLatch.release();
            rootIN.releaseLatch();
            for (SplitInfo info : nodeLadder) {
                if (info.child != binResult) {
                    info.child.releaseLatch();
                }
                if (info.newChild != null && info.newChild != binResult) {
                    info.newChild.releaseLatch();
                }
            }
            ...
            assert binResult.isLatchExclusiveOwner();

            return (BIN) binResult;
            ...
            for (SplitInfo info : nodeLadder) {
                ...
                if (info.newChild != null) {
                    info.newChild.releaseLatchIfOwner();
                }
            }
            if (child != null) {
    ...
    /*
     * Like forceSplitLog, except that:
     *  - We only log the split node siblings (in the IN class), not any of
     *    its ancestor INs. This reduces logging.
     *  - We don't latch the ancestor INs during the split process. Only the
     *    a single child-parent pair are latched at a time, using latch.
     *    This reduces blocking.
     * After this method has been thoroughly debugged and tested, it should be
     * renamed to forceSplit and the forceSplitLog method should be removed.
     * Also remove the grandParent param of IN split methods. Also remove
     * extra recovery pass that is a workaround for the split aunt problem.
     *
     * For now, however, there are failures in CheckSplitAuntTest indicating
     * that logging of the split siblings is causing the split aunt problem.
     * But without logging the siblings we may end up with a null LSN in the
     * slot of a logged parent IN. This requires more time to investigate.
     *
     * Also note that the latching in this method is out of sync with changes
     * in forceSplitLog and this should be corrected before using this method.
     */
    private BIN forceSplitNoLog(byte[] key,
                                CacheMode cacheMode,
                                InternalComparator comparator)
        throws SplitRequiredException {
        ...
        try {
            ...
            do {
                ...
                if (child.needsSplitting()) {
                    ...
                    final IN origChild = child;

                    child = findSplitBranch(
                        parent, key, comparator, child, newSibling);

                    final IN otherChild = (child == origChild) ?
                        newSibling : origChild;

                    otherChild.releaseLatch();
                }
    ...
    private IN findSplitBranch(IN parent,
                               byte[] key,
                               InternalComparator comparator,
                               IN child1,
                               IN child2) {

        final int index = parent.findEntry(key, false, false, comparator);
        final IN child = (IN) parent.getTarget(index);

        if (child != child1 && child != child2) {
            throw EnvironmentFailureException.unexpectedState();
        }
    ...
    private void splitRoot(CacheMode cacheMode)
        ...
        try {
            ...
            newRoot = new IN(
                database, rootIdKey, curRoot.entryKeys.getCompactMaxKeySize(),
                maxTreeEntriesPerNode, curRoot.getLevel() + 1);
            ...
            try {
                LogItem curRootItem = curRoot.logProvisional(newRoot);
                curRootLsn = curRootItem.lsn;

                int inserted = newRoot.insertEntry1(
                    curRoot, rootIdKey, null, curRootLsn, false);
                assert (inserted & IN.INSERT_SUCCESS) != 0;

                newRoot.setLastLoggedSize(
                    inserted & ~IN.INSERT_SUCCESS, curRootItem.size);

                logLsn = newRoot.log().lsn;
                logSuccess = true;
            } finally {
                if (!logSuccess) {
            ...
            IN newSibling = curRoot.split(
                newRoot, 0, null, -1, maxTreeEntriesPerNode);
    ...
    public void readFromLog(EnvironmentImpl envImpl,
        ...
        if (rootExists) {
            ...
            root.readFromLog(envImpl, itemBuffer, entryVersion);
        }
    }
