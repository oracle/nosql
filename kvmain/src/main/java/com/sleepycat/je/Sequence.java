...
public class Sequence implements Closeable {
    ...
    Sequence(Database db,
        ...
        try {
            locker = LockerFactory.getReadableLocker(db, txn);
            ...
            if (sequenceExists) {
                ...
                if (useConfig.getAllowCreate()) {
                    ...
                    /*
                     * Since we don't have phantom detection, try
    ...
    private boolean readData(Cursor cursor, LockMode lockMode)
        ...
        byte flags = buf.get();
        rangeMin = LogUtils.readPackedLong(buf);
        rangeMax = LogUtils.readPackedLong(buf);
        storedValue = LogUtils.readPackedLong(buf);
