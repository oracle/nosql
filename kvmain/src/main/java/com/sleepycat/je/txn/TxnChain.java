...
public class TxnChain {
    ...
    private long lastValidVLSN = INVALID_VLSN;
    ...
    public TxnChain(
        ...
        try {
            lastValidVLSN = NULL_VLSN;

            while (currLsn != DbLsn.NULL_LSN) {

                /*
                 * LastLoggedSize param is -1 meaning we'll have to do two
                 * fetches. This is OK because rollback is infrequent.
                 */
                WholeEntry wholeEntry =
                    logManager.getLogEntryAllowInvisible(currLsn, -1);
                ...
                try {
                    ...
                    if (DbLsn.compareTo(currLsn, matchpoint) > 0) {
                        ...
                        if (lastValidVLSN != INVALID_VLSN &&
                            VLSN.isNull(lastValidVLSN) &&
                            wholeEntry.getHeader().getVLSN() != INVALID_VLSN &&
                            !VLSN.isNull(wholeEntry.getHeader().getVLSN())) {
