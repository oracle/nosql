...
public abstract class Locker {
    ...
    private final long txnStartMillis;    // for txn timeout determination
    ...
    /**
     * The name for thread that created this locker. Used for debugging. Note
     * that name may be null if the Locker is instantiated by reading the log.
     */
    ...
    protected Locker(EnvironmentImpl envImpl,
        ...
        /* Check the default txn timeout. */
        txnTimeoutMillis = envImpl.getTxnTimeout();

        txnStartMillis = TimeSupplier.currentTimeMillis();
     ...
     * throwIfPreempted, so that ThreadLocker.checkPreempted can call
     * throwIfPreempted for all ThreadLockers for the current thread without
     * causing an infinite recursion.
     */
    public void checkPreempted()
     ...
     * @see #checkPreempted
     */
    final void throwIfPreempted()
        throws OperationFailureException {

        if (preemptedCause != null) {
            throw new LockPreemptedException(this, preemptedCause);
        }
    }
    ...
    public LockResult postLogNonBlockingLock(long lsn,
        ...
        return nonBlockingLock(
            lsn, lockType, jumpAheadOfWaiters, database, cursor);
    }
     ...
     */
    public synchronized void demoteLock(long lsn, CursorImpl unused)
        throws DatabaseException {
     ...
     * such as cleaner LN migration.  We must acquire a lock on the new LSN on
     * behalf of every locker that currently holds a lock on the old LSN.
     *
    ...
    public synchronized void lockAfterLsnChange(long oldLsn,
        ...
        final LockResult lockResult = nonBlockingLock
            (newLsn, lockType, false /*jumpAheadOfWaiters*/, dbImpl, null);
        ...
        /*
         * Note that null was passed above for the cursor. We must associate
         * the new LSN with _all_ cursors with which the old LSN is associated.
         */
        copyCursorLocks(oldLsn, newLsn);
    }
     ...
     */
    public abstract void releaseNonTxnLocks(CursorImpl cursor)
        throws DatabaseException;
     ...
     */
    public abstract void nonTxnOperationEnd(CursorImpl cursor)
        throws DatabaseException;
    ...
    public boolean isTimedOut() {
        ...
        if (timeout != 0) {
            long diff = TimeSupplier.currentTimeMillis() - txnStartMillis;
            if (diff > timeout) {
                return true;
            }
        }
    ...
    public String toString() {
        ...
        return System.identityHashCode(this) + " " + Long.toString(id) + "_" +
               ((threadName == null) ? "" : threadName) + "_" +
               className;
    }

    /** See {@link Txn#copyCursorLocks}. */
    public void copyCursorLocks(long oldLsn, long newLsn) {
        /* Do nothing */
    }

    /** See {@link Txn#transferCursorLocks}. */
    public void transferCursorLocks(CursorImpl oldCursor,
                                    CursorImpl newCursor) {
        /* Do nothing. */
    }
