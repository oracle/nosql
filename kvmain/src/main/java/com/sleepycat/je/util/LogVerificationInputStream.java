...
public class LogVerificationInputStream extends InputStream {
    ...
    /**
     * Used when an EnvironmentImpl is available rather than an Environment.
     */
    public LogVerificationInputStream(final EnvironmentImpl envImpl,
                                      final InputStream in,
                                      final String fileName) {
        this(envImpl, in, fileName, -1L);
    }
    ...
    /**
     * Perform verification for when the stream EOF is reached.
     */
    protected void verifyAtEof() throws LogVerificationException {
        verifier.verifyAtEof();
    }

    /**
     * Verify the specified bytes.
     */
    protected void verify(final byte[] b, final int off, final int len)
        throws LogVerificationException
    {
        verifier.verify(b, off, len);
    }
