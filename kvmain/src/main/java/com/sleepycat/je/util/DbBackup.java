...
public class DbBackup {
    ...
    private int networkRestoreClientId;
    ...
    /* For unit tests. */
    private TestHook<?> testEndBackupHook;
    private TestHook<?> testStartWindowHook;
     ...
     * is wedged or starved. (This exception is undocumented because erasure
     * is not an exposed feature.)
     * -->
     */
    public synchronized void startBackup()
        ...
        if (!envImpl.addDbBackup(this)) {
            throw new LogOverwriteException(
         ...
         * select two reserved files, giving preference to recent files
         * containing VLSNs. The reserved files have two purposes:
         * 1. The rep stream portion of the log (covered by the VLSNIndex) must
         *    not be allowed to become empty on the restored node. If there are
         *    no active files in the rep stream then we must include at least
         *    the most recent reserved file that is part of the rep stream.
         *    Otherwise the VLSNIndex will become empty when it is truncated on
         *    the restored node during recovery [KVSTORE-467].
         * 2. Sending some reserved files is safeguard against other edge
         *    conditions that might prevent the restored node from performing
         *    recovery or functioning as a master.
         ...
         * last file, aborting erasure and protecting the active files.
         */
        final ProtectedFileRange allFilesProtected =
            fileProtector.protectFileRange(
                backupName + FileProtector.TEMP_SUFFIX, 0);

        try {
            /*
             * In rare cases it is possible for the active log files to not
             * contain the last sync point, or any replicated logs at all.  If
             * that happens then restoring from the backup will fail because it
             * would require truncating the only possible matchpoint form the
             * VLSNIndex.  As such find the file containing the matchpoint and
             * make sure it is part of the backup set.
             * See KVSTORE-1931
             */
            long matchpointFile = DbLsn.NULL_LSN;
            if (envImpl.isReplicated()) {
                final RepImpl repImpl = (RepImpl) envImpl;
                final VLSNIndex vlsnIndex = repImpl.getVLSNIndex();
                long lastSync = vlsnIndex.getRange().getLastSync();
                if (lastSync != DbLsn.NULL_LSN) {
                    matchpointFile = vlsnIndex.getLTEFileNumber(lastSync);
                }
            }
            /*
             * Abort erasure of all files while all files are protected, so
             * none of the files we select for the backup are in danger of
             * being erased.
             */
            envImpl.getDataEraser().abortErase(allFilesProtected);

            /*
             * Protect active files from deletion prior to the file flip. This
             * includes files prior to firstFileInBackup, in order to get a
             * snapshot of all active files. New files (following those in the
             * final backup set) do not need protection, since the backup set
            ...
            if (envIsReadOnly) {
                 ...
                 */
                final LogEntry marker = new SingleItemEntry<>(
                    LogEntryType.LOG_IMMUTABLE_FILE, new EmptyLogEntry());

                final long markerLsn = envImpl.getLogManager().log(
            ...
            assert TestHookExecute.doHookIfSet(testStartWindowHook);

            /*
             * Protect files following those initially protected up to and
             * including lastFileInBackup. See addFinalBackupFiles for details.
             */
            protectedFileSet.addFinalBackupFiles(lastFileInBackup);
        } finally {
    ...
    public synchronized void endBackup() {
        ...
        assert TestHookExecute.doHookIfSet(testEndBackupHook);
    ...
    /**
     * Equivalent to {@link #getLogFilesInBackupSet} but returns file numbers.
     * Intended for testing.
     */
    public synchronized Set<Long> getFileNumbersInBackupSet() {
        checkBackupStarted();
        return new HashSet<>(snapshotFiles);
    }
     ...
     */
    public void setNetworkRestore(int clientId) {
        networkRestore = true;
        networkRestoreClientId = clientId;
    }
     ...
     */
    public void setEndBackupHook(TestHook<?> testHook) {
        this.testEndBackupHook = testHook;
    }
     ...
     * @hidden
     *
     * A test entry point in the startBackup window between flipping the file
     * and getting the list of active files.
     */
    public void setStartWindowHook(TestHook<?> testHook) {
