...
public class DataVerifier {
    ...
    public static TestHook<VerifySummary> verifyHook;
    public static TestHook<VerifyLogSummary> logVerifyHook;

    private static final String VERIFIER_SCHEDULE =
        "test.je.env.verifierSchedule";
    ...
    public DataVerifier(EnvironmentImpl envImpl) {
        ...
        this.timer = new Timer(
            ...
            true /*isDaemon*/);
        dbLogVerifier = new DbVerifyLog(envImpl, 0, null);
    ...
    public class VerifyTask extends TimerTask {
        ...
        private void updateConfig() {
            ...
            dbLogVerifier.setBackground(true);
            ...
            dbTreeVerifier.setBackground(true);
        ...
        public void run() {
             ...
             */
            if (TimeSupplier.currentTimeMillis() - scheduledExecutionTime() >=
            ...
            updateConfig();
            try {
                /* Note that both verifyAll methods log start/end/warnings. */
                if (verifyLog) {
                    final VerifyLogSummary summary = dbLogVerifier.verifyAll();
                    if (logVerifyHook != null) {
                        logVerifyHook.doHook(summary);
                    }
                }
                if (verifyBtree) {
                    final VerifySummary summary = dbTreeVerifier.verifyAll();
                    if (verifyHook != null) {
                        verifyHook.doHook(summary);
                    }
                }
                success = true;
