 ...
 *
 * TRACK_LATCHES is set to true if the system property je.trackLatches is
 * defined to true. This enables related debugging methods used to check the
 * number of Btree latches held.
 *
 * CAPTURE_OWNER is also set to true if the system property
 * je.trackLatchOwner is defined to true.  This will capture a stack trace
...
public class LatchSupport {
    ...
    public static final boolean TRACK_LATCHES =
        Boolean.getBoolean("je.trackLatches");

    static final boolean CAPTURE_OWNER =
        Boolean.getBoolean("je.trackLatchOwner");
    ...
    public static void dumpOtherLatchesHeld() {
        System.out.println(otherLatchesHeldToString());
    }

    public static String otherLatchesHeldToString() {
        return otherLatchTable.latchesHeldToString();
    }
    ...
    static String debugString(final Latch latch,
        ...
        builder.append(TimeSupplier.currentTimeMillis());
    ...
    static EnvironmentFailureException handleTimeout(
        final SemaphoreLatchContext context) {

        final EnvironmentImpl envImpl = context.getEnvImplForFatalException();
        final Logger logger = envImpl.getLogger();
        final String name = context.getLatchName();

        LoggerUtils.logMsg(
            logger, envImpl, Level.SEVERE,
            "Thread dump follows for SemaphoreLatch timeout: " + name);

        LoggerUtils.fullThreadDump(logger, envImpl, Level.SEVERE);

        return unexpectedState(
            envImpl, "SemaphoreLatch timeout. " + name);
