...
public class LongDiffStat extends MapStatComponent<Long, LongDiffStat> {
    ...
    public void set(long newValue) {
        set(newValue, TimeSupplier.currentTimeMillis());
    }
    ...
    @Override
    public Long get() {
        return get(TimeSupplier.currentTimeMillis());
    }
    ...
    protected synchronized String getFormattedValue(boolean useCommas) {
        if (isNotSet()) {
            ...
            return FormatUtil.decimalScale0().format(
                get(TimeSupplier.currentTimeMillis()));
        } else {
            return String.valueOf(get(TimeSupplier.currentTimeMillis()));
        }
    }
