...
public abstract class DaemonThread implements DaemonRunner, Runnable {
    ...
    public static final String ERROR_LISTENER = "setErrorListener";
    ...
    public void runOrPause(boolean run, FIOStatsCollector collector) {
        if (run) {
            ...
            if (thread != null) {
                ...
                thread =
                    new FileStatsCollectingThread(this, name, collector);
    ...
    public final class FileStatsCollectingThread extends Thread
    implements FIOStatsCollectingThread {

        public FIOStatsCollector collector;

        FileStatsCollectingThread(Runnable r, String name,
                                  FIOStatsCollector collector) {
            super(r, name);
            setDaemon(true);
    ...
    public boolean checkErrorListener(Throwable e) {
        if (Boolean.getBoolean(ERROR_LISTENER)) {
            if (!stifleExceptionChatter) {
                logger.severe(name + " " + LoggerUtils.getStackTraceForSevereLog(e));
            }
            @SuppressWarnings("unused")
            EnvironmentFailureException ignore = new EnvironmentFailureException
