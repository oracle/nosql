...
import static com.sleepycat.je.cleaner.FileProtector.FEEDER_NAME;
import static com.sleepycat.je.cleaner.FileProtector.NETWORK_RESTORE_NAME;
...
public class Cleaner implements DaemonRunner, EnvConfigObserver {
    ...
    private volatile String diskUsageMessage;
     ...
     */
    @Override
    public void runOrPause(boolean run, FIOStatsCollector collector) {
        ...
        for (FileProcessor processor : threads) {
            ...
            processor.runOrPause(run, collector);
        }
    }
    ...
    public void fakeDaemonDoClean() {
        createProcessor().doClean(true, true, true);
    }
    ...
    /**
     * Compares the long values of two map entries.
     */
    private static Comparator<Map.Entry<?, Long>>
        MAP_ENTRY_LONG_VALUE_COMPARATOR = (e1, e2) ->
            (e1.getValue() > e2.getValue()) ? 1 :
            (e1.getValue() < e2.getValue()) ? (-1) : 0;

    /**
     * Filters out entries that are protected only momentarily, since they are
     * insignificant when finding a feeder to kill.
     */
    private Stream<Map.Entry<String, Long>> getNonTempEntries(
        final Map<String, Long> protectedSizeMap) {

        return protectedSizeMap.entrySet().stream()
            .filter(e -> !e.getKey().endsWith(FileProtector.TEMP_SUFFIX));
    }

    /**
     * Returns the protected map entry with the largest value, among those with
     * the given name prefix, if any.
     */
    private Optional<Map.Entry<String, Long>> getFeederMaxSizeEntry(
        final Map<String, Long> protectedSizeMap,
        final String namePrefix) {

        final String prefix = namePrefix + "-";

        return getNonTempEntries(protectedSizeMap)
            .filter(e -> e.getKey().startsWith(prefix))
            .max(MAP_ENTRY_LONG_VALUE_COMPARATOR);
    }

    /**
     * Returns the protected map entry with the largest value, among those with
     * a non-feeder name prefix, if any.
     */
    private Optional<Map.Entry<String, Long>> getNonFeederMaxSizeEntry(
        final Map<String, Long> protectedSizeMap) {

        final String prefix1 = FEEDER_NAME + "-";
        final String prefix2 = NETWORK_RESTORE_NAME + "-";

        return getNonTempEntries(protectedSizeMap)
            .filter(e ->
                !e.getKey().startsWith(prefix1) &&
                !e.getKey().startsWith(prefix2))
            .max(MAP_ENTRY_LONG_VALUE_COMPARATOR);
    }

    /**
     * Attemps to kill the feeder for the replica with the given map entry.
     */
    private boolean killReplica(
        final Map.Entry<String, Long> feederEntry,
        final Optional<Map.Entry<String, Long>> optOtherEntry) {

        assert feederEntry.getKey().startsWith(FEEDER_NAME + "-");

        final String nameIdPair =
            feederEntry.getKey().substring(FEEDER_NAME.length() + 1);
        final String replicaName = NameIdPair.getNameFromString(nameIdPair);

        if (optOtherEntry.isPresent()) {
            final Map.Entry<String, Long> otherEntry = optOtherEntry.get();
            final long otherBytesSaved = otherEntry.getValue();
            final long bytesSaved = feederEntry.getValue();

            if (otherBytesSaved >= bytesSaved) {
                LoggerUtils.info(logger, env, "Killing a replica will not" +
                    " free enough bytes as other sources are protecting" +
                    " files. Replica=" + replicaName +
                    ", replicaProtectedBytes=" + bytesSaved +
                    ", otherEntity=" + otherEntry.getKey() +
                    ", otherEntityProtectedBytes=" + otherBytesSaved);
                return false;
            }
        }

        final RepImpl repImpl = (RepImpl)env;
        final FeederManager feederManager =
            repImpl.getRepNode().feederManager();
        // Can happen during testing.
        if (feederManager == null) {
            LoggerUtils.info(logger, env,
                "Cannot kill lagging replica, FeederManager is null: " +
                replicaName);
            return false;
        }
        final Feeder feeder = feederManager.getFeeder(replicaName);
        // May have already been shut down.
        if (feeder == null) {
            LoggerUtils.info(logger, env,
                "Cannot kill lagging replica, not in active feeder list: " +
                replicaName);
            return false;
        }

        LoggerUtils.warning(logger, env,
            "Killing lagging replica to avoid out-of-disk: " +
            replicaName);

        feederManager.shutdownFeeder(feeder.getReplicaNode());
        return true;
    }

    /**
     * Attempts to kill the feeder for the network restore with the given map
     * entry.
     */
    private boolean killNetworkRestore(
        final Map.Entry<String, Long> feederEntry,
        final Optional<Map.Entry<String, Long>> optOtherEntry) {

        assert feederEntry.getKey().startsWith(NETWORK_RESTORE_NAME + "-");

        final String feederName =
            feederEntry.getKey().substring(NETWORK_RESTORE_NAME.length() + 1);
        final int clientId = Integer.parseInt(feederName);
        ...
        if (optOtherEntry.isPresent()) {
            ...
            if (otherBytesSaved >= bytesSaved) {
                LoggerUtils.info(logger, env,
                    "Killing a network restore will not" +
                    " free enough bytes as other sources are protecting" +
                    " files. networkRestoreClientId=" + clientId +
                    ", networkRestoreProtectedBytes=" + bytesSaved +
                    ", otherEntity=" + otherEntry.getKey() +
                    ", otherEntityProtectedBytes=" + otherBytesSaved);
                return false;
            }
        ...
        final RepImpl repImpl = (RepImpl)env;
        final com.sleepycat.je.rep.impl.networkRestore.FeederManager
            feederManager = repImpl.getRepNode().getLogFeederManager();
        // May be shut down during syncup
        if (feederManager == null) {
            LoggerUtils.info(logger, env,
                "Cannot kill network restore, FeederManager is null: " +
                clientId);
            return false;
        }
        if (!feederManager.isClientActive(clientId)) {
            LoggerUtils.info(logger, env,
                "Cannot kill network restore, clientId no longer active: " +
                clientId);
            return false;
        }

        LoggerUtils.warning(logger, env,
            "Killing network restore to avoid out-of-disk, clientId: " +
            clientId);

        feederManager.shutdownClient(clientId);
        return true;
    }

    /**
     * Attempt to truncate the VLSNIndex then delete unprotected files to
     * regain disk space.  If no bytes were recovered then pick a network
     * restore or lagging replica to kill and try again.  Should only be called
     * when holding the manageDiskUsageLock.
     */
    private long truncateVLSNIndexAndDelete(
        long bytesNeeded, final long originalBytes)
        throws EnvLockedException {

        if (env.tryVlsnHeadTruncate(bytesNeeded)) {
            bytesNeeded = deleteUnprotectedFiles(bytesNeeded);
        }

        /*
         * If no bytes have been reclaimed yet then look for a network restore
         * or lagging replica to kill to allow bytes to be reclaimed.
         */
        if (bytesNeeded != originalBytes) {
            return bytesNeeded;
        }
        LoggerUtils.info(logger, env,
            "Attempting to kill a replica or network restore to enable" +
            " deletion of reserved files and prevent an out-of-disk" +
            " condition.");

        /*
         * Search the protected files for the active feeder that is
         * preventing the most bytes from being deleted.  Only active
         * feeders doing replay are killed, nascent feeders before the
         * replay stage are ignored because shutting them down is awkward
         * and finding the matchpoint is a quick process.
         */
        final Optional<Map.Entry<String, Long>> replicaToKill =
            getFeederMaxSizeEntry(
                logSizeStats.protectedSizeMap, FEEDER_NAME);

        final Optional<Map.Entry<String, Long>> networkRestoreToKill =
            getFeederMaxSizeEntry(
                logSizeStats.protectedSizeMap, NETWORK_RESTORE_NAME);

        if (!replicaToKill.isPresent() && !networkRestoreToKill.isPresent()) {
            LoggerUtils.info(logger, env,
                "No replica or network restore found to kill.");
            return bytesNeeded;
        }

        final Optional<Map.Entry<String, Long>> otherEntry =
            getNonFeederMaxSizeEntry(logSizeStats.protectedSizeMap);

        /*
         * Kill a network restore in preference to a replica, since the replica
         * is already running.
         */
        if ((networkRestoreToKill.isPresent() &&
             killNetworkRestore(networkRestoreToKill.get(), otherEntry)) ||
            (replicaToKill.isPresent() &&
             killReplica(replicaToKill.get(), otherEntry))) {
        ...
        try {
            ...
            if (fileProtector.getNReservedFiles() > 0) {
                ...
                if (env.isReplicated()) {
                     ...
                     */
                    if (bytesNeeded > 0) {
                        bytesNeeded = truncateVLSNIndexAndDelete(
                            bytesNeeded, origBytesNeeded);
                        freshenStats = true;
                    }
    ...
    private long deleteUnprotectedFiles(long bytesNeeded)
        ...
        try {
            ...
            if (!deletedFiles.isEmpty()) {
                ...
                sb.append(INT_FORMAT.format(getDiskFreeSpace()));

                if (!expunge) {
                    sb.append(" NOTE: files were renamed not deleted," +
    ...
    private boolean deleteFile(final Long file) {
        ...
        try {
            if (expunge) {
                ...
                if (newFile != null) {
                    newFile.setLastModified(TimeSupplier.currentTimeMillis());
                    return true;
                }
            }
        } catch (IOException e) {
     ...
     * single-threaded recovery or tests.
     */
    ...
    void recalcLogSizeStats(final FileProtector.LogSizeStats stats,
        ...
        if (availBytes <= 0) {
            sb.append("Disk usage is not within the ");
            if (maxOverage > 0 && freeShortage > 0) {
                sb.append("je.maxDisk and je.freeDisk limits");
            } else if (maxOverage > 0) {
                sb.append("je.maxDisk limit");
            } else {
                assert freeShortage > 0;
                sb.append("je.freeDisk limit");
            }
            sb.append(" and write operations are prohibited:");
        } else {
            ...
            if (pendingLNs != null) {
                ...
                for (final Map.Entry<Long, LNInfo> entry :
                    ...
                    try {
                        ...
                        if (env.getExtinctionStatus(db, key) == EXTINCT) {
                            fileSelector.removePendingLN(logLsn);
    ...
    private void processPendingLN(
        ...
        try {
            ...
            final LockResult lockRet = locker.nonBlockingLock(
                logLsn, LockType.READ, false /*jumpAheadOfWaiters*/, db, null);
            ...
            if (bin.getFetchedCold()) {
                ...
                db.getEnv().getEvictor().doCacheModeEvict(
    ...
    static ReplicationContext getMigrationRepContext(LN ln) {
        ...
        return new ReplicationContext(vlsnSeq,
