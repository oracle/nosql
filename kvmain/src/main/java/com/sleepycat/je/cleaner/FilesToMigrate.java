...
class FilesToMigrate {
    ...
    boolean hasNext(SortedMap<Long, FileSummary> fileSummaryMap,
                    Set<Long> inProgressFiles, long lastFileToClean) {
        if (nextAvailable) {
        ...
        for (long file :
            ...
            /*
             * Skip the file if it is being cleaned or
             * if it is too young to be cleaned.
             */
            if (inProgressFiles.contains(file) || file > lastFileToClean) {
                continue;
            }
    ...
    long next(SortedMap<Long, FileSummary> fileSummaryMap,
        ...
        if (hasNext(fileSummaryMap, inProgressFiles, lastFileToClean)) {
            nextAvailable = false;
            return nextFile;
        } else {
