...
public class Transaction {
     ...
     * com.sleepycat.je.EnvironmentConfig#setDurability}
    ...
    public synchronized void commitSync()
        ...
        doCommit(Durability.COMMIT_SYNC);
    }
    ...
    public synchronized void commitNoSync()
        ...
        doCommit(Durability.COMMIT_NO_SYNC);
    }
     ...
     * otherwise configured using the {@link
     * com.sleepycat.je.EnvironmentConfig#setDurability
     * EnvironmentConfig.setDurability} method.  This behavior may also be set
     * for a single transaction using the {@link
     * com.sleepycat.je.Environment#beginTransaction
     * Environment.beginTransaction} method.  Any value specified to this
     * method overrides both of those settings.</p>
    ...
    public synchronized void commitWriteNoSync()
        ...
        doCommit(Durability.COMMIT_WRITE_NO_SYNC);
    }
     ...
     */
    private void doCommit(Durability durability) {
        try {
            checkEnv();
            checkOpen();
            env.removeReferringHandle(this);
     ...
     *
     * <p>If {@link #setTxnTimeout(long,TimeUnit)} has been called to configure
     * the timeout, that value is returned.  Otherwise, if {@link
     * TransactionConfig#setTxnTimeout} was called on the {@code
     * TransactionConfig} specified when creating the transaction, then that
     * value is returned.  Otherwise, the environment configuration value
     * ({@link EnvironmentConfig#TXN_TIMEOUT}) is returned.</p>
     *
    ...
    /**
     * Nulls-out indirect references to the environment, to allow GC.
     * <p>
     * The app may hold the Transaction references longer than expected. In
     ...
     * <p>
     * Note that this is needed even when the txn or env is invalid.
     */
