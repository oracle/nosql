 ...
 *
 *         cursor1 = secondaryDb1.openCursor(txn, null);
 *         cursor2 = secondaryDb2.openCursor(txn, null);
 *
...
public class JoinCursor implements ForwardCursor, Closeable {
    ...
    JoinCursor(final Database primaryDb,
        ...
        if (!config.getNoSort()) {
            ...
            for (int i = 0; i < cursors.length; i += 1) {
                counts[i] = cursors[i].countEstimateInternal(false);
                assert counts[i] >= 0;
            }
        ...
        outerLoop: while (true) {
            ...
            for (int i = 1; i < secCursors.length; i += 1) {
                ...
                result = secCursor.search(
                    scratchEntry, candidateKey, lockMode,
                    false /*excludeTombstones*/, cacheMode,
            ...
            final boolean dataRequested = dataParam != null &&
                (!dataParam.getPartial() || dataParam.getPartialLength() > 0);

            if (dataRequested) {
                result = secCursors[0].readPrimaryAfterGet(
                ...
                dataParam.setData(LogUtils.ZERO_LENGTH_BYTE_ARRAY);
            }
