...
public class ReplicatedDatabaseConfig extends BasicVersionedWriteLoggable {
    ...
    public DatabaseConfig getReplicaConfig(EnvironmentImpl envImpl) {
        ...
        if (reader.isClass()) {
            if (reader.isByteComparator()) {
                replicaConfig.setBtreeByteComparator(
                    reader.getByteComparatorClass());
            } else {
        ...
        if (reader.isClass()) {
            if (reader.isByteComparator()) {
                replicaConfig.setDuplicateByteComparator(
                    reader.getByteComparatorClass());
            } else {
                replicaConfig.setDuplicateComparator(
                    reader.getComparatorClass());
            }
        } else {
            if (reader.isByteComparator()) {
                replicaConfig.setDuplicateByteComparator(
    ...
    public void readFromLog(EnvironmentImpl envImpl,
        ...
        maxTreeEntriesPerNode = LogUtils.readPackedInt(itemBuffer);
