...
import static com.sleepycat.je.utilint.JETaskCoordinator.JE_INCOMPRESSOR_TASK;
...
public class INCompressor extends DaemonThread {
    ...
    /*
     * Do not have a timeout for the amount of time
     ...
     */
    public synchronized void doCompress(boolean invokedFromDaemon)
        throws DatabaseException {
        ...
        try (Permit permit = (!invokedFromDaemon ? null
            : envImpl.getTaskCoordinator().acquirePermit(JE_INCOMPRESSOR_TASK,
            ...
            for (final BINReference binRef : queueSnapshot.values()) {
                ...
                } else if (binSearch.bin != null) {
                    if (binSearch.bin.getFetchedCold()) {
                        /* This releases the latch. */
                        envImpl.getEvictor().doCacheModeEvict(
                            binSearch.bin, CacheMode.UNCHANGED);
                    } else {
    ...
    private void compressBin(
        ...
        boolean exceptionInFlight = false;
        ...
        try {
            ...
            if (!exceptionInFlight && bin.getFetchedCold()) {
                /* This releases the latch. */
                envImpl.getEvictor().doCacheModeEvict(
                    bin, CacheMode.UNCHANGED);
            } else {
                bin.releaseLatch();
            }
        }
    ...
    public void lazyCompress(final IN in, final boolean compressDirtySlots) {
        ...
        if (bin.compress(compressDirtySlots)) {
    ...
    private boolean findDBAndBIN(
         ...
         */
        binSearch.db = dbTree.getDb(binRef.getDatabaseId(), dbCache);
