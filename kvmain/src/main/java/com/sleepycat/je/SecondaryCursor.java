 ...
 * SecondaryDatabase.openCursor}. The distinguishing characteristics
 ...
 *
 * <blockquote><pre>
 *     SecondaryCursor cursor = myDb.openCursor(txn, null);
 * </pre></blockquote>
 *
 ...
 *     SecondaryCursor cursor = myDb.openCursor(txn, config);
 * </pre></blockquote>
 */
public class SecondaryCursor extends Cursor {
    ...
    public OperationResult delete(final WriteOptions options) {
         ...
         */
        final OperationResult secResult = getCurrentInternal(
            key, pKey, LockMode.RMW, false /*excludeTombstones*/, cacheMode);
        ...
        throw secondaryDb.secondaryRefersToMissingPrimaryKey(
            true, locker, primaryDb, key, pKey, secResult.getExpirationTime());
    }
        ...
        if (searchMode != null) {
            ...
            if (searchMode.isDataSearch() ||
                (searchMode == SearchMode.ANY_RANGE &&
                 getDatabaseImpl().getSortedDuplicates())) {
    ...
    OperationResult search(final DatabaseEntry key,
        ...
        final OperationResult result1 = search(
            ...
            cacheMode, searchMode, true);
        if (result1 == null) {
            return null;
        }

        trace(Level.FINEST, "SecondaryCursor.search", key, pKey,
              "\nIndex entry after Cursor search. SearchMode = " + searchMode);
        ...
        trace(Level.FINE, "SecondaryCursor.search", key, pKey,
              "\nSkipping index entry. SearchMode = " + searchMode);
     ...
     *
     * In order to avoid deadlocks we change the natural lock order for reads:
     * we only lock the primary record and then check the secondary record's
     * reference to primary record. The initial read of the secondary DB is
    ...
    private OperationResult readPrimaryAfterGet(
        ...
        if (dataRequested) {
            ...
            /*
             * The expiration time may have changed after a secondary
             * dirty-read and before locking the primary.
             */
            return secDirtyRead ?
