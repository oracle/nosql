...
import static com.sleepycat.je.utilint.JETaskCoordinator.JE_CHECKPOINTER_TASK;
...
public class Checkpointer extends DaemonThread implements EnvConfigObserver {
     ...
     */
    private static TestHook<CheckpointReference> maxFlushLevelHook = null;

    private static TestHook<CheckpointReference> beforeNodeFlushHook = null;
     ...
     */
    public void coordinatePruningWithCheckpoint(final IN branchParent) {
        checkpointDirtyMap.coordinatePruningWithCheckpoint(branchParent);
    }
    ...
    public synchronized void doCheckpoint(CheckpointConfig config,
        ...
        try {
            ...
            LoggerUtils.info(envImpl.getLogger(), envImpl,
                "Checkpoint attempt failed because " +
                    "disk space limits have been reached.");
        ...
        final DataEraser eraser = envImpl.getDataEraser();
        ...
        /* Log a message after generating the checkpoint id above. */
        LoggerUtils.info(envImpl.getLogger(), envImpl,
            "Checkpoint Start." + "Checkpoint Id=" + checkpointId);
        ...
        try (Permit permit = (!invokedFromDaemon ? null
            : envImpl.getTaskCoordinator().acquirePermit(JE_CHECKPOINTER_TASK,
                PERMIT_WAIT_MS, PERMIT_TIME_TO_HOLD_MS, TimeUnit.MILLISECONDS,
                handler))) {
            ...
            final CheckpointEnd ckptEnd = new CheckpointEnd(
                ...
                checkpointId, willDeleteFiles, completedScans);
            ...
            eraser.flushSuspectObsoleteData();
            ...
            LoggerUtils.info(envImpl.getLogger(), envImpl,
                "Checkpoint End." + " Checkpoint Id=" + checkpointId +
                    " success=" + success);
    ...
    public static void setMaxFlushLevelHook(TestHook<CheckpointReference> hook) {
        maxFlushLevelHook = hook;
    }

    /* For unit testing only. */
    public static void setBeforeNodeFlushHook(TestHook<CheckpointReference> hook) {
        beforeNodeFlushHook = hook;
    }
    ...
    private static void flushIN(final DatabaseImpl db,
        ...
        assert (targetLevel < maxFlushLevel) ||
            TestHookExecute.doHookIfSet(maxFlushLevelHook, targetRef);

        assert (TestHookExecute.doHookIfSet(beforeNodeFlushHook, targetRef));
        ...
        try {
            ...
            if (!result.exactParentFound) {
                if (parentLevel > targetLevel) {
                    dirtyMap.addIN( parent, false /*updateFlushLevels*/);
                }
                return;
            }
            ...
            /*
             * Determine whether our search found the IN identified by
             * targetRef.lsn. If there is not a match, then the node was
             * deleted, logged or split since creating the reference.
             */
            assert targetRef.lsn != DbLsn.NULL_LSN;
            ...
            for (int i = 0; i < parent.getNEntries(); i += 1) {
                ...
                final CheckpointReference childRef =
                    dirtyMap.removeNode(targetLevel, childLsn);
    ...
    private static void logDirtyIN(
        ...
        try {
            ...
            assert (logItem.lsn != DbLsn.NULL_LSN);

            isBIN = child.isBIN();
            isDelta = (logItem.lsn == child.getLastDeltaLsn());

            /*
             * If the identifier key of the child was changed, change the
             * corresponding slot in the parent.
             * Note: Don't update if a delta was logged for the child.
             * otherwise we would fail to find the corresponding full bin
             * during tree lookup in cleaner.
             */
            if (!logItem.deltaBinLogged) {
                parent.updateChildKeyWhenLogged(index, child);
            }
        } finally {
            child.releaseLatch();
        }

        parent.updateEntry(index, logItem.lsn, NULL_VLSN, logItem.size);
    ...
    public static class CheckpointReference {
        ...
        public CheckpointReference(final DatabaseId dbId,
                                   final long nodeId,
                                   final int nodeLevel,
                                   final boolean isRoot,
                                   final byte[] treeKey,
