...
public class LatencyStat implements Cloneable {
    ...
    private synchronized Latency calculate(boolean clear) {
         ...
         * incremented last.
         */
        final long totalOps = values.numOps.get();
        final long totalRequests = values.numRequests.get();
        ...
        final long nTrackedRequests = totalRequests - nOverflow;
        if (nTrackedRequests == 1) {
            ...
            percent95Count = (long) (nTrackedRequests * .95);
            percent99Count = (long) (nTrackedRequests * .99);
        }
