...
import oracle.kv.impl.api.table.serialize.parsing.ResolvingGrammarGenerator;
 ...
 *
 * <p>
 * See the <a href="doc-files/parsing.html">parser documentation</a> for
...
public class ResolvingDecoder extends ValidatingDecoder {
    ...
    ResolvingDecoder(FieldDefImpl writer, FieldDefImpl reader, Decoder in)
            throws IOException {
        this(resolve(writer, reader), in);
    }

    /**
     * Constructs a <tt>ResolvingDecoder</tt> using the given resolver. The
     * resolver must have been returned by a previous call to
     * {@link #resolve(FieldDefImpl, FieldDefImpl)}.
     * 
     * @param resolver
     *            The resolver to use.
     * @param in
     ...
     * 
     * @param writer
     *            The writer's FieldDefImpl. Cannot be null.
     * @param reader
     *            The reader's FieldDefImpl. Cannot be null.
     * @return The opaque reolver.
     * @throws IOException
     */
    public static Object resolve(FieldDefImpl writer, FieldDefImpl reader)
            throws IOException {
        if (null == writer) {
    ...
    /**
     * Returns the actual order in which the reader's fields will be returned to
     * the reader.
     *
     * This method is useful because {@link ResolvingDecoder} returns values in
     * the order written by the writer, rather than the order expected by the
     * reader. This method allows readers to figure out what fields to expect.
     * Let's say the reader is expecting a three-field record, the first field
     * is a long, the second a string, and the third an array. In this case, a
     * typical usage might be as follows:
     * 
     * <pre>
     *   FieldMapEntry[] fieldOrder = in.readFieldOrder();
     *   for (int i = 0; i &lt; 3; i++) {
     *     switch (i) {
     *     case 1:
     *       foo(in.readLong());
     ...
     * </pre>
     * 
     * Note that {@link ResolvingDecoder} will return only the fields expected
     * by the reader, not other fields that may have been written by the writer.
     * Thus, the iteration-count of "3" in the above loop will always be
     * correct.
     *
     * Throws a runtime exception if we're not just about to read the field of a
     * record. Also, this method will consume the field information, and thus
     * may only be called <em>once</em> before reading the field value.
     * (However, if the client knows the order of incoming fields, then the
     * client does <em>not</em> need to call this method but rather can just
     * start reading the field values.)
     *
     * @throws SerializationRuntimeException
     *             If we're not starting a new record
     * 
     */
    public final FieldMapEntry[] readFieldOrder() throws IOException {
        return ((Symbol.FieldOrderAction) parser
                .advance(Symbol.FIELD_ACTION)).fields;
    }

    /**
     * Consume any more data that has been written by the writer but not needed
     * by the reader so that the the underlying decoder is in proper shape for
     * the next record. This situation happens when, for example, the writer
     * writes a record with two fields and the reader needs only the first
     * field.
     * 
     * This function should be called after completely decoding an object but
     * before next object can be decoded from the same underlying decoder either
     * directly or through another resolving decoder. If the same resolving
    ...
    public int readEnum() throws IOException {
        ...
        Symbol.EnumAdjustAction top = (Symbol.EnumAdjustAction) parser
                .popSymbol();
        int n = in.readEnum();
        ...
        throw new SerializationRuntimeException((String) o);
    }

    @Override
    public int readIndex() throws IOException {
        parser.advance(Symbol.UNION);
        Symbol.UnionAdjustAction top = (Symbol.UnionAdjustAction) parser
                .popSymbol();
        ...
        if (top instanceof Symbol.ResolvingAction) {
            ...
            if (t.reader != input) {
                throw new SerializationRuntimeException(
                        "Found " + t.reader + " while looking for " + input);
            }
            return t.writer;
            ...
            Symbol.Alternative branches = (Symbol.Alternative) parser
                    .popSymbol();
            parser.pushSymbol(branches.getSymbol(in.readIndex()));
        } else if (top instanceof Symbol.ErrorAction) {
            throw new SerializationRuntimeException(((Symbol.ErrorAction) top).msg);
            ...
            throw new SerializationRuntimeException("Unknown action: " + top);
        }
        return null;
    }
