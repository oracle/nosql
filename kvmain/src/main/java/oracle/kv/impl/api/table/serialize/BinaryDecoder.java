...
public class BinaryDecoder extends Decoder {
    ...
    BinaryDecoder configure(byte[] data, int offset, int length) {
        configureSource(DecoderFactory.DEFAULT_BUFFER_SIZE,
                new ByteArrayByteSource(data, offset, length));
        return this;
    }

    /**
     * Initializes this decoder with a new ByteSource. Detaches the old source
     * (if it exists) from this Decoder. The old source's state no longer
     * depends on this Decoder and its InputStream interface will continue to
     * drain the remaining buffer and source data.
     * <p/>
     ...
     */
    private void configureSource(int bufferSize, ByteSource byteSource) {
        if (null != this.source) {
            this.source.detach();
        }
        byteSource.attach(bufferSize, this);
    ...
    protected void doReadBytes(byte[] bytes, int start, int length)
        ...
        if (length < 0)
            throw new RuntimeException(
                    "Malformed data. Length is negative: " + length);
        int remaining = limit - pos;
        if (length <= remaining) {
     ...
     * Returns 0 if there are no more items in the current array and the
     * array/map has ended.
     *
     * @throws IOException
     */
     ...
     * skipped (because byte count to skip is not found in the stream) return
     ...
     * data and cannot read any further without throwing an EOFException or
     * other IOException.
     * <p/>
     ...
     * However, buf[pos + num -1] may be >= limit if there is not enough data
     * left in the source to fill the array with num bytes.
     * <p/>
     ...
     * reads are valid by checking that their read did not advance past the
     * limit before adjusting pos.
     * <p/>
    ...
    /**
     * Returns an {@link java.io.InputStream} that is aware of any buffering
     * that may occur in this BinaryDecoder. Readers that need to interleave
     * decoding data with other reads must access this InputStream to do so
     * unless the implementation is 'direct' and does not read beyond the
     * minimum bytes necessary from the source.
     */
    ...
    /**
     * BufferAccessor is used by AvroEncoder to enable {@link ByteSource}s and
     * the InputStream returned by {@link BinaryDecoder#inputStream} to access
     * the AvroEncoder's buffer. When a BufferAccessor is created, it is
     * attached to a BinaryDecoder and its buffer. Its accessors directly
     * reference the BinaryDecoder's buffer. When detach() is called, it stores
     * references to the BinaryDecoder's buffer directly. The BinaryDecoder only
     * detaches a BufferAccessor when it is initializing to a new ByteSource.
     * Therefore, a client that is using the InputStream returned by
     * BinaryDecoder.inputStream can continue to use that stream after a
     * BinaryDecoder has been reinitialized to read from new data.
     */
     ...
     * InputStream does not in general require that as many bytes as possible
     * have been read when filling a buffer.
     * <p/>
     * InputStream's terminating conditions for a read are two-fold:
     * EOFException and '-1' on the return from read(). Implementations are
     * supposed to return '-1' on EOF but often do not. The extra terminating
     * conditions cause extra conditionals on both sides of the API, and slow
     * performance significantly.
     * <p/>
     * ByteSource implementations provide read() and skip() variants that have
     * stronger guarantees than InputStream, freeing client code to be
     * simplified and faster.
     * <p/>
     * {@link #skipSourceBytes} and {@link #readRaw} are guaranteed to have
     * read or skipped as many bytes as possible, or throw EOFException. {@link
     * #trySkipBytes} and {@link #tryReadRaw} are guaranteed to attempt to
     * read or skip as many bytes as possible and never throw EOFException,
     * while returning the exact number of bytes skipped or read. {@link isEof}
     * returns true if all the source bytes have been read or skipped. This
     * condition can also be detected by a client if an EOFException is thrown
     * from {@link #skipSourceBytes} or {@link #readRaw}, or if
     * {@link #trySkipBytes} or {@link #tryReadRaw} return 0;
     * <p/>
     * A ByteSource also implements the InputStream contract for use by APIs
     * that require it. The InputStream interface must take into account
     * buffering in any decoder that this ByteSource is attached to. The other
     * methods do not account for buffering.
     */
    ...
    abstract static class ByteSource extends InputStream {
        ...
        /**
         * Skips length bytes from the source. If length bytes cannot be skipped
         * due to end of file/stream/channel/etc an EOFException is thrown
         *
         * @param length
         ...
         * actual number of bytes skipped. This method must attempt to skip as
         * many bytes as possible up to <i>skipLength</i> bytes. Skipping 0
         * bytes signals end of stream/channel/file/etc
         *
         * @param skipLength
         *            the number of bytes to attempt to skip
         * @return the count of actual bytes skipped.
         */
        protected abstract long trySkipBytes(long skipLength)
                throws IOException;

        /**
         * Reads raw from the source, into a byte[]. Used for reads that are
         * larger than the buffer, or otherwise unbuffered. This is a mandatory
         * read -- if there is not enough bytes in the source, EOFException is
         * thrown.
         *
         * @throws IOException
         *             if an error occurs
         ...
         * starting at index <i>off</i>. Returns the actual number of bytes
         * copied which may be between 0 and <i>len</i>.
         * <p/>
         ...
         * <i>remaining</i> bytes starting at <i>pos</i> at <i>minPos</i>. This
         * may be done in the current buffer, or may replace the buffer with a
         * new one.
         *
         * The end result must be a buffer with at least 16 bytes of remaining
         * space.
         *
         * @param pos
         * @param minPos
         ...
         * @throws IOException
         */
        protected void compactAndFill(byte[] buf, int pos, int minPos,
                int remaining) throws IOException {
            System.arraycopy(buf, pos, buf, minPos, remaining);
            ba.setPos(minPos);
            int newLimit = remaining + tryReadRaw(buf, minPos + remaining,
    ...
    private static class InputStreamByteSource extends ByteSource {
        ...
        @Override
        protected void readRaw(byte[] data, int off, int len)
                throws IOException {
            while (len > 0) {
    ...
    private static class ByteArrayByteSource extends ByteSource {
        ...
        protected long trySkipBytes(long length) throws IOException {
            ...
            max = ba.getLim();
            position = ba.getPos();
            final int remaining = max - position;
            if (remaining >= length) {
                position += (int) length;
                ba.setPos(position);
                return length;
        ...
        @Override
        protected void readRaw(byte[] dataValue, int off, int len)
                throws IOException {
            int read = tryReadRaw(dataValue, off, len);
            if (read < len) {
                throw new EOFException();
            }
        }
        ...
        @Override
        protected int tryReadRaw(byte[] dataValue, int off, int len)
                throws IOException {
        ...
        protected void compactAndFill(byte[] buf, int pos, int minPos,
            ...
            // so it makes a new tiny buffer unless it has been compacted once
            // before
            if (!compacted) {
                // assumes ensureCapacity is never called with a size more than
                // 16
