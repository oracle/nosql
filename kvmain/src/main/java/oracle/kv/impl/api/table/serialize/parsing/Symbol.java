...
public abstract class Symbol {
     ...
     * order of production. This is useful for easy copying onto parsing stack.
     *
     * Please note that this is a final. So the production for a symbol should
     * be known before that symbol is constructed. This requirement cannot be
     * met for those symbols which are recursive (e.g. a record that holds union
     * a branch of which is the record itself). To resolve this problem, we
     * initialize the symbol with an array of nulls. Later we fill the symbols.
     * Not clean, but works. The other option is to not have this field a final.
     * But keeping it final and thus keeping symbol immutable gives some
     * confort. See various generators how we generate records.
     */
     ...
     * Every <tt>Sequence</tt> in the input are replaced by its production
     * recursively. Non-<tt>Sequence</tt> symbols, they internally have other
     * symbols those internal symbols also get flattened.
     *
     * The algorithm does a few tricks to handle recursive symbol definitions.
     * In order to avoid infinite recursion with recursive symbols, we have a
     * map of Symbol&rarr;Symbol. Before fully constructing a flattened symbol
     * for a <tt>Sequence</tt> we insert an empty output symbol into the map
     * and then start filling the production for the <tt>Sequence</tt>. If the
     * same <tt>Sequence</tt> is encountered due to recursion, we simply return
     * the (empty) output <tt>Sequence<tt> from the map. Then we actually fill
     * out the production for the <tt>Sequence</tt>. As part of the flattening
     * process we copy the production of <tt>Sequence</tt>s into larger arrays.
     * If the original <tt>Sequence</tt> has not not be fully constructed yet,
     * we copy a bunch of <tt>null</tt>s. Fix-up remembers all those
     * <tt>null</tt> patches. The fix-ups gets finally filled when we know the
     * symbols to occupy those patches.
     *
     * @param in
     *            The array of input symbols to flatten
     * @param start
     *            The position where the input sub-array starts.
     * @param out
     *            The output that receives the flattened list of symbols. The
     *            output array should have sufficient space to receive the
     *            expanded sub-array of symbols.
     * @param skip
     *            The position where the output input sub-array starts.
     * @param map
     *            A map of symbols which have already been expanded. Useful for
     *            handling recursive definitions and for caching.
     * @param map2
     *            A map to to store the list of fix-ups.
     */
    static void flatten(Symbol[] in, int start, Symbol[] out, int skip,
            Map<Sequence, Sequence> map, Map<Sequence, List<Fixup>> map2) {
    ...
    protected static class ImplicitAction extends Symbol {
        /**
         * Set to <tt>true</tt> if and only if this implicit action is a
         * trailing action. That is, it is an action that follows real symbol.
         * E.g {@link Symbol#DEFAULT_END_ACTION}.
         */
    ...
    protected static class Root extends Symbol {
        ...
        private static Symbol[] makeProduction(Symbol[] symbols) {
            ...
            flatten(symbols, 0, result, 1, new HashMap<Sequence, Sequence>(),
    ...
    protected static class Repeater extends Symbol {
        ...
        public Repeater flatten(Map<Sequence, Sequence> map,
                Map<Sequence, List<Fixup>> map2) {
            Repeater result = new Repeater(end,
                    new Symbol[flattenedSize(production, 1)]);
    ...
    /**
     * Returns true if the Parser contains any Error symbol, indicating that it
    ...
    public static EnumAdjustAction enumAdjustAction(int rsymCount,
            Object[] adj) {
    ...
    protected static SkipAction skipAction(Symbol symToSkip) {
    ...
    public static FieldAdjustAction fieldAdjustAction(int rindex,
            String fname) {
    ...
    public static FieldOrderAction fieldOrderAction(FieldMapEntry[] fields) {
        return new FieldOrderAction(fields);
    }

    public static final class FieldOrderAction extends ImplicitAction {
        public final FieldMapEntry[] fields;

        @Deprecated
        public FieldOrderAction(FieldMapEntry[] fields) {
            this.fields = fields;
        }
    }
    ...
    public static final Symbol CRDT_START = new Symbol.Terminal("crdt-start");
    public static final Symbol CRDT_END = new Symbol.Terminal("crdt-end");

    /* a pseudo terminal used by parsers */
    public static final Symbol FIELD_ACTION = new Symbol.Terminal(
            "field-action");
    ...
    public static final Symbol MAP_KEY_MARKER = new Symbol.Terminal(
            "map-key-marker");
}
