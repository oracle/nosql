...
public class Parser {
    /**
     * The parser knows how to handle the terminal and non-terminal symbols. But
     * it needs help from outside to handle implicit and explicit actions. The
     * clients implement this interface to provide this help.
     */
    ...
    public Parser(Symbol root, ActionHandler symbolHandler) {
        ...
        this.stack = new Symbol[5]; // Start small to make sure expansion code
                                    // works
        this.stack[0] = root;
        this.pos = 1;
    }
     ...
     * production, until the top is a terminal. Then checks if the top symbol
     * matches the terminal symbol suppled <tt>terminal</tt>.
     * 
     * @param input
     *            The symbol to match against the terminal at the top of the
     *            stack.
     * @return The terminal symbol at the top of the stack unless an implicit
     *         action resulted in another symbol, in which case that symbol is
     *         returned.
     */
    public final Symbol advance(Symbol input) throws IOException {
        for (;;) {
            ...
            if (k == Symbol.Kind.IMPLICIT_ACTION) {
                ...
                throw new SerializationRuntimeException("Attempt to process a "
     ...
     * production (other than the root) that may be encountered. This method
     * will fail if there are any repeaters on the stack.
     * 
     * @throws IOException
     */
    ...
    /**
     * Pushes the production for the given symbol <tt>sym</tt>. If <tt>sym</tt>
     * is a repeater and <tt>input</tt> is either {@link Symbol#ARRAY_END} or
     * {@link Symbol#MAP_END} pushes nothing.
     * 
     * @param sym
     */
    protected final void pushProduction(Symbol sym) {
