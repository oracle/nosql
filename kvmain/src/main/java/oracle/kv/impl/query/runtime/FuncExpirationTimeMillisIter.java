 ...
 *
 */
public class FuncExpirationTimeMillisIter extends SingleInputPlanIter {
    ...
    public boolean next(RuntimeControlBlock rcb) {
        ...
        if (row.isTuple()) {
            ...
            throw new QueryException(
                "Input to the expiration_time_millis() function is not a row",
                getLocation());
        }

        rcb.setRegVal(theResultReg,
                      FieldDefImpl.Constants.longDef.createLong(expTime));
        state.done();
        return true;
    }
