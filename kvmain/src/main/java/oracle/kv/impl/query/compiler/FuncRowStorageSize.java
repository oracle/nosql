...
import oracle.kv.impl.query.runtime.FuncRowStorageSizeIter;
import oracle.kv.impl.query.runtime.PlanIter;
import oracle.kv.impl.query.types.TypeManager;

/**
 * Function to return the current storage row size
 */
public class FuncRowStorageSize extends Function {

    FuncRowStorageSize() {
        super(FuncCode.FN_ROW_STORAGE_SIZE, "row_storage_size",
    ...
    Expr normalizeCall(ExprFuncCall funcCall) {
        ...
        if (arg.getKind() == ExprKind.VAR &&
            ((ExprVar)arg).getTable() != null) {
            return funcCall;
        }

        throw new QueryException(
            "The argument to the row_storage_size function must " +
    ...
    PlanIter codegen(CodeGenerator codegen,
        ...
        return new FuncRowStorageSizeIter(caller, resultReg, argIters[0]);
    }
}
