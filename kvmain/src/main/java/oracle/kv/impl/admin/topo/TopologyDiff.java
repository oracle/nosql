...
public class TopologyDiff {
    ...
    /** Map of changes to zone rep factor. */
    private final Map<DatacenterId, Integer> changedZoneRepFactors;

    /* Map of changes to zone master affinity */
    private final Map<DatacenterId, Boolean> changedAffinityZones;

    /** Map of changes to whether zones allow arbiters. */
    private final Map<DatacenterId, Boolean> changedAllowArbiterZones;
     ...
     */
    public TopologyDiff(Topology source,
                        String sourceName,
    ...
    /**
     * Evaluate the differences between the source and destination, or
     * candidate, and optionally validate the transition.
     *
     * @param source the original topology
     * @param sourceName the candidate name of the original topology. If null,
     * the default name of CURRENT will be used.
     * @param candidate the destination topology.
     * @param params the store parameters
     * @param validate whether to validate the changes
     */
    public TopologyDiff(Topology source,
                        String sourceName,
                        TopologyCandidate candidate,
                        Parameters params,
                        boolean validate) {
        this(source, sourceName, candidate, params, validate,
             false /* forFailover */);
    }

    /**
     * Evaluate the differences between the source and destination, or
     * candidate, optionally validate the transition, and specify whether the
     * changes are part of a failover
     *
     * @param source the original topology
     * @param sourceName the candidate name of the original topology. If null,
     * the default name of CURRENT will be used.
     ...
     * @param params the store parameters
     * @param validate whether to validate the changes
     * @param forFailover whether the changes are part of a failover
     */
    public TopologyDiff(Topology source,
                        String sourceName,
                        TopologyCandidate candidate,
        ...
        changedZoneRepFactors = new HashMap<>();
        changedAffinityZones = new HashMap<>();
        changedAllowArbiterZones = new HashMap<>();
        newRNs = new HashSet<>();
    ...
    /**
     * Returns the amount the replication factor of the zone changed, or 0 for
     * no change.
     */
    public int repFactorChange(DatacenterId zoneId) {
        return changedZoneRepFactors.getOrDefault(zoneId, 0);
    }
    ...
    /**
     * Returns true if allowing arbiters changed for the specified zone.
     */
    public boolean allowArbitersChanged(DatacenterId zoneId) {
        return changedAllowArbiterZones.containsKey(zoneId);
    }
     ...
     *
     * Change N zone types
     * Change N zone replication factors
     * Change N zone master affinities
     * Change N zone allow arbiter properties
     * Create N shards
     * Create N RNs
     * Remove N RNs
     * Create N partitions
     ...
     * Remove N ANs
     *
     * change ZN zone type to TYPE
     * change ZN replication factor by RF_CHANGE
     * change ZN master affinity to TRUE_OR_FALSE
     * change ZN allow arbiters to TRUE_OR_FALSE
     * shard X [(to be removed)]
     *  Y new RNs: rg7-rn1 rg7-rn2 rg7-rn3
     *  Z partition migrations
     *  N to be removed RNs: rg1-rn1 rg2-rn1
     *  Relocate rg1-rn1 from sn1 storagedir=foo rnlogdir=bar to sn2
     *           storagedir=baz rnlogdir=quux
     *  N new ANs: rg1-an1
     *  Relocate rg1-an1 from sn1 to sn2
     *  N remove ANs: rg1-an1
     *  N new partitions
     *
    ...
    public String display(boolean verbose) {
        ...
        if (!changedZoneRepFactors.isEmpty()) {
            sb.append("Change ").append(changedZoneRepFactors.size())
                .append(" zone replication factor")
                .append(plural(changedZoneRepFactors.size()))
                .append("\n");
        }
        ...
        if (!changedAffinityZones.isEmpty()) {
            sb.append("Change ").append(changedAffinityZones.size()).
                append(" zone master ").
                append(changedAffinityZones.size() == 1 ?
                       "affinity" :
                       "affinities").
                append("\n");
        }

        if (!changedAllowArbiterZones.isEmpty()) {
            sb.append("Change ").append(changedAllowArbiterZones.size())
                .append(" zone allow arbiter ")
                .append(changedAllowArbiterZones.size() == 1 ?
                        "property" :
                        "properties")
                .append("\n");
        }
        ...
        for (Entry<DatacenterId, DatacenterType> e : changedZones.entrySet()) {
            sb.append("change ").append(e.getKey()).
                append(" zone type to ").append(e.getValue()).append("\n");
        }

        changedZoneRepFactors.forEach(
            (k, v) ->
            sb.append("change ").append(k)
            .append(" replication factor by ").append(v).append("\n"));
        ...
        changedAllowArbiterZones.forEach(
            (k, v) ->
            sb.append("change ").append(k)
            .append(" allow arbiters to ").append(v).append("\n"));
    ...
    public ObjectNode displayJson(boolean verbose) {
        ...
        if (!changedZoneRepFactors.isEmpty()) {
            top.put("changeZoneRepFactorsTotal", changedZoneRepFactors.size());
        }
        ...
        if (!changedAllowArbiterZones.isEmpty()) {
            top.put("changeAllowArbiterZonesTotal",
                    changedAllowArbiterZones.size());
        }
        ...
        final ArrayNode changeZoneRepFactorsArray =
            top.putArray("changeZoneRepFactors");
        changedZoneRepFactors.forEach((key, val) -> {
                final ObjectNode on = JsonUtils.createObjectNode();
                on.put("change", key.toString());
                on.put("by", val.toString());
                changeZoneRepFactorsArray.add(on);
            });
        ...
        final ArrayNode changeAllowArbiterZonesArray =
            top.putArray("changeAllowArbiterZones");
        changedAllowArbiterZones.forEach((key, val) -> {
                final ObjectNode on = JsonUtils.createObjectNode();
                on.put("change", key.toString());
                on.put("to", val.toString());
                changeAllowArbiterZonesArray.add(on);
            });
     ...
     */
    private void doDiff(boolean validate, boolean forFailover) {
         ...
         */
        if (validate) {
            Rules.validateTransition(source, candidate, params, forFailover);
        }
    ...
    private void findChangedZones() {
        for (Datacenter dc :
            ...
            if (oldDc.getRepFactor() != dc.getRepFactor()) {
                changedZoneRepFactors.put(
                    dc.getResourceId(),
                    dc.getRepFactor() - oldDc.getRepFactor());
            }
            if (oldDc.getMasterAffinity() != dc.getMasterAffinity()) {
                changedAffinityZones.put(dc.getResourceId(),
                                         dc.getMasterAffinity());
            }
            if (oldDc.getAllowArbiters() != dc.getAllowArbiters()) {
                changedAllowArbiterZones.put(dc.getResourceId(),
                                             dc.getAllowArbiters());
            }
        }
    }
    ...
    public static class ShardChange {
        ...
        public Set<RepNodeId> getRemovedRNs() {
            return removedRNs;
        }
    ...
    public static class RelocatedAN {
        ...
        @Override
        public String toString() {
            return "Relocate " + arbId + " from " + oldSNId + " to " + newSNId;
        }
    }
